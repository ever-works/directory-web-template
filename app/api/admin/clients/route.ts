...
import { createClientProfile, getClientProfiles, getUserByEmail } from '@/lib/db/queries';
...
export async function GET(request: NextRequest) {
	try {
		...
		if ('error' in paginationResult) {
			return NextResponse.json({ error: paginationResult.error }, { status: paginationResult.status });
		}
...
export async function POST(request: NextRequest) {
	try {
		...
		let user: Awaited<ReturnType<typeof getUserByEmail>> | AuthUserData = await getUserByEmail(
			email,
		...
		if (!user || !user.id) {
			return NextResponse.json({ error: 'Failed to create or find user for client' }, { status: 400 });
		}
		...
		if (crmEnabled) {
			try {
				...
				const { mapClientProfileToPerson } = await import('@/lib/mappers/twenty-crm.mapper');
