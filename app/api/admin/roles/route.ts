...
export async function GET(request: NextRequest) {
	try {
		...
		const status: RoleStatus | undefined =
			statusParam === 'active' || statusParam === 'inactive' ? (statusParam as RoleStatus) : undefined;
...
export async function POST(request: NextRequest) {
	try {
		...
		if (!id) {
			return NextResponse.json(
				{ success: false, error: 'Unable to derive a valid role ID from name' },
				{ status: 400 }
			);
		}
