...
export async function GET(request: NextRequest) {
	try {
		...
		const queryParams = {
			...
			interval:
				intervalParam && validIntervals.includes(intervalParam)
					? (intervalParam as SponsorAdIntervalType)
					: undefined,
		...
		const result = await sponsorAdService.getSponsorAdsPaginated(tenantId, validationResult.data);
		console.log('[Admin Sponsor Ads] Query result:', JSON.stringify(result, null, 2));

		// Get stats for dashboard
		const stats = await sponsorAdService.getSponsorAdStats(tenantId);
