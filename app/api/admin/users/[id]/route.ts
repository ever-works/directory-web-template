...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
	try {
		...
		const user = await userRepository.findById(id, session.user.tenantId);
	...
	try {
		...
		if (body.title !== undefined && body.title !== null) {
			...
			if (body.title.length > 100) {
				return NextResponse.json(
					{ success: false, error: 'Title must be at most 100 characters' },
					{ status: 400 }
				);
			}
		}
		...
		if (body.avatar !== undefined && body.avatar !== null) {
			...
			if (body.avatar.length > 500) {
				return NextResponse.json(
					{ success: false, error: 'Avatar URL must be at most 500 characters' },
					{ status: 400 }
				);
			}
		}
		...
		if (body.role !== undefined) {
			...
			const roleExists = await roleRepository.findById(body.role, session.user.tenantId);
			if (!roleExists) {
		...
		// Update user
		const userRepository = new UserRepository();
		const updatedUser = await userRepository.update(id, userData, session.user.tenantId);
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
	try {
		...
		// Delete user
		const userRepository = new UserRepository();
		await userRepository.delete(id, session.user.tenantId);
