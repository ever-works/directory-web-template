...
async function getLatestCommit(contentPath: string, branch?: string): Promise<GitCommit> {
	try {
...
function createVersionInfo(commit: GitCommit, branch: string, repository: string | undefined): VersionInfo {
...
export async function GET() {
	...
	try {
		...
		// Step 5: Get latest commit information
		const latestCommit = await getLatestCommit(contentPath, branch);
