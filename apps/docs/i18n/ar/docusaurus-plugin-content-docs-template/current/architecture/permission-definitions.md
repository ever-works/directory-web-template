...
title: تعريفات الإذن
sidebar_label: تعريفات الإذن
sidebar_position: 32
---

# تعريفات الإذن

يتضمن القالب نظام أذونات دقيق منظم في تعريفات قائمة على الموارد، ومجموعات صديقة لواجهة المستخدم، وأدوات مساعدة لإدارة الحالة. يتحكم هذا النظام في الوصول إلى ميزات الإدارة وإدارة المحتوى والتحليلات.
...
## تعريفات الأذونات (`definitions.ts`)

تتبع كافة الأذونات اصطلاح التسمية `resource:action`. الكائن `PERMISSIONS` هو المصدر الوحيد للحقيقة:
...
يتم اشتقاق النوع `Permission` تلقائيًا من الكائن `PERMISSIONS`، مما يضمن سلامة النوع دون النسخ اليدوي:
...
يوفر تعريفان مضمنان للأدوار افتراضيات معقولة:
...
## مجموعات الأذونات (`groups.ts`)

تنظم مجموعات الأذونات الأذونات للعرض في مكونات واجهة المستخدم الإدارية مثل محرري الأدوار:
...
### وظائف الأداة المساعدة للمجموعة
...
## الأدوات المساعدة لحالة الإذن (`utils.ts`)

تدير هذه الوظائف حالة تبديل الأذونات في واجهات المستخدم الإدارية (مثل إنشاء الأدوار أو تحرير النماذج):

### إنشاء حالة الإذن

تحويل مصفوفة الأذونات إلى خريطة منطقية لواجهات المستخدم المستندة إلى مربع الاختيار:
...
تحديد ما تمت إضافته وإزالته عند مقارنة مجموعات الأذونات:
...
تحقق مما إذا كانت صفيفتا الأذونات تحتويان على نفس الأذونات بغض النظر عن الترتيب:
...
البحث من خلال الأذونات عن طريق الكلمات الرئيسية:
...
### التحقق من الأذونات في أحد المكونات
...
- `lib/permissions/definitions.ts` - ثوابت الأذونات وأنواعها والأدوار الافتراضية
- `lib/permissions/groups.ts` - مجموعات واجهة المستخدم ومساعدي التنسيق
- `lib/permissions/utils.ts` - أدوات إدارة الدولة
- `lib/guards/` - حراس المسار والمكونات الذين يستهلكون الأذونات
