...
title: مرجع هيكل المشروع
sidebar_label: هيكل المشروع
sidebar_position: 4
---

# مرجع هيكل المشروع

يوفر هذا المستند مرجعًا كاملاً لبنية دليل قالب Ever Works. القالب عبارة عن **Turborepo monorepo** تتم إدارته باستخدام مساحات عمل pnpm. التطبيق الأساسي هو تطبيق Next.js 16 الذي يستخدم App Router وTypeScript وDrizzle ORM وTailwind CSS، الموجود في `apps/web/`.

## نظرة عامة على المستوى الأعلى (جذر Monorepo)
...
التطبيق الأساسي موجود في `apps/web/`. جميع أدلة المصدر التي كانت موجودة سابقًا في جذر المستودع موجودة الآن هنا.
...
يستخدم القالب **Turborepo** لتنسيق البناء و**مساحات عمل pnpm** لإدارة التبعية عبر monorepo.

|أداة|الغرض|التكوين|
|------|---------|---------------|
|توربوريبو|تنسيق المهام، والتخزين المؤقت، والبنيات المتوازية|`turbo.json`|
|مساحات عمل pnpm|إدارة التبعية، وربط مساحة العمل|`pnpm-workspace.yaml`|
|`packages/tsconfig`|تكوينات قاعدة TypeScript المشتركة|ممتدة بواسطة كل تطبيق|
|`packages/eslint-config`|إعدادات التكوين المسبقة المسطحة لـ ESLint 9 المشتركة|يتم استيرادها بواسطة كل تطبيق|

### تطبيقات مساحة العمل

- **`apps/web`** - تطبيق Next.js 16 الرئيسي (قائمة الدليل، لوحة الإدارة، مسارات واجهة برمجة التطبيقات).
- **`apps/web-e2e`** - اختبارات الكاتب المسرحي الشاملة التي تستهدف `apps/web`. مفصولة في مساحة العمل الخاصة بها من أجل التبعيات المعزولة وعمليات تشغيل CI المستقلة.
- **`apps/docs`** -- موقع توثيق Docusaurus v3 للقالب نفسه.
