...
تدير فئة `ConfigManager` في `lib/config-manager.ts` ملف `works.yaml` من مستودع نظام إدارة المحتوى المعتمد على Git. تتعامل مع القراءة والكتابة وتأكيد تغييرات التهيئة.
