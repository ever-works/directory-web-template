...
title: "التعليقات مخطط الغوص العميق"
sidebar_label: "مخطط التعليقات"
sidebar_position: 54
---

# التعليقات مخطط الغوص العميق

## نظرة عامة

يمكّن نظام التعليقات المستخدمين من ترك تعليقات ومراجعات على العناصر. ترتبط التعليقات بـ `client_profiles` (وليس مباشرة بـ `users`)، وتتضمن حقل تصنيف، وتدعم الحذف المبدئي عبر `deletedAt`. يوفر النظام الفرعي للإشراف (`reports` و`moderation_history`) تقارير عن المحتوى وتتبع الإجراءات الإدارية.
...
نظام الإبلاغ عن المحتوى لكل من العناصر والتعليقات.
...
|`contentType`|`content_type`|`text (enum)`|لا| - |`item`، `comment`|
|`contentId`|`content_id`|`text`|لا| - |معرف المحتوى المبلغ عنه|
|`reason`|`reason`|`text (enum)`|لا| - |انظر التعداد أدناه|
|`details`|`details`|`text`|نعم| - |التفاصيل المقدمة من قبل المستخدم|
|`status`|`status`|`text (enum)`|لا|`'pending'`|انظر التعداد أدناه|
|`resolution`|`resolution`|`text (enum)`|نعم| - |انظر التعداد أدناه|
|`reportedBy`|`reported_by`|`text`|لا| - |FK -> `client_profiles.id` (CASCADE)|
|`reviewedBy`|`reviewed_by`|`text`|نعم| - |FK -> `users.id` (الضبط فارغ)|
|`reviewNote`|`review_note`|`text`|نعم| - |مذكرة مراجعة المشرف|
|`createdAt`|`created_at`|`timestamp`|لا|`now()`| - |
|`updatedAt`|`updated_at`|`timestamp`|لا|`now()`| - |
|`reviewedAt`|`reviewed_at`|`timestamp`|نعم| - | - |
|`resolvedAt`|`resolved_at`|`timestamp`|نعم| - | - |
...
|الاسم|أعمدة|اكتب|
|---|---|---|
|`reports_content_type_idx`|`contentType`|شجرة ب|
|`reports_content_id_idx`|`contentId`|شجرة ب|
|`reports_status_idx`|`status`|شجرة ب|
|`reports_reported_by_idx`|`reportedBy`|شجرة ب|
|`reports_created_at_idx`|`createdAt`|شجرة ب|
|`reports_content_type_content_id_idx`|`(contentType, contentId)`|شجرة B المركبة|
