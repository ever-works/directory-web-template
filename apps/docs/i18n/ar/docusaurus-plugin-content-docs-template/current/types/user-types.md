---
id: user-types
title: تعريفات نوع المستخدم والملف الشخصي
sidebar_label: أنواع المستخدمين
sidebar_position: 2
---

# تعريفات نوع المستخدم والملف الشخصي

**المصدر:** `lib/types/user.ts` و`lib/types/profile.ts`

تحدد هذه الوحدات أنواع المستخدمين الإداريين (المصادقة والإدارة) وملفات تعريف المستخدمين العامة (العرض والمحفظة).
...
بيانات المستخدم للمصادقة فقط المخزنة في الجدول `users`. هذا هو الحد الأدنى من تمثيل المستخدم المستخدم لتدفقات المصادقة.
...
بيانات المستخدم الكاملة التي تتضمن معلومات الملف الشخصي من `clientProfiles`. هذا هو تمثيل المستخدم الكامل المستخدم في لوحة تحكم المشرف.
...
**الفرق عن `AuthUserData`:** في `UserData`، `username`، `name`، `role`، `status`، و`created_by` كلها مطلوبة (غير اختيارية).

#### `CreateUserRequest`

الحمولة لإنشاء مستخدم إداري جديد.
...
الحمولة لتحديث مستخدم حالي. جميع الحقول اختيارية.
...
استجابة مرقّمة لقوائم مستخدم المصادقة.
...
استجابة مرقّمة لقوائم المستخدمين الكاملة.
...
معلمات الاستعلام لتصفية قوائم المستخدمين وترقيم صفحاتها.
...
بيانات المستخدم الموسعة مع حقل عدد اختياري للإحصائيات.
...
مخطط التحقق الكامل لإنشاء مستخدم:
...
مخطط جزئي للتحديثات (باستثناء كلمة المرور):
...
اكتب Guard الذي يضيق السلسلة إلى `UserStatus`:
...
تنسيق `Date` كسلسلة متوافقة مع YAML (`YYYY-MM-DD HH:mm`):
