...
title: API клиентска архитектура
...
Това ръководство обхваща двойната API клиентска система: клиентската `ApiClient` (базиран на Axios сингълтон) и сървърната `ServerClient` (базиран на извличане с кеширане и повторни опити), включително безопасност на типа, обработка на грешки и прехващачи на заявка/отговор.
...
## API клиент от страна на клиента

### Единичен модел `ApiClient` използва строг сингълтон, управляван от `ApiClientSingleton` :
...
### Прехващачи на заявки

Клиентът автоматично прикачва токена на носителя към всяка заявка:
...
### Прехващачи на отговор

Автоматичната обработка на 401 пренасочва към страницата за вход:
...
Всички грешки се нормализират до последователна форма `ApiError` :
...
### ServerClient клас `ServerClient` в `lib/api/server-api-client.ts` е оптимизиран за използване от страна на сървъра:
...
Заявките GET се кешират автоматично с изгонване на LRU:
...
Всяка заявка обгръща извличането с AbortController:
...
### Предварително изградени клиентски екземпляри
...
### Обработка на безопасен тип отговор
...
### Конфигурация на заявката по подразбиране
...
1. **Singleton осигурява един екземпляр на Axios**: Избягва претоварването на връзката от създаването на множество клиенти.
2. **Кеширане от страна на сървъра**: Намалява излишните извиквания на API. Деактивирайте за работни потоци с тежки мутации.
3. **React Query staleTime**: 5 минути предотвратява повторното извличане при всяко монтиране на компонент.
4. **gcTime от 24 часа**: Запазва данните в паметта за бърза навигация между страниците.
5. **Брой повторения от 1**: Балансира устойчивостта със закъснението, обърнато към потребителя.

## Отстраняване на неизправности

### Заявките от страна на клиента са неуспешни с 401

1. Проверете дали токенът за достъп е зададен на екземпляр `ApiClient` .
2. Проверете дали прихващачът на токени прикачва заглавката `Authorization` .
3. Проверете дали токенът не е изтекъл.

### Кешът от страна на сървъра връща остарели данни

1. Извикайте `serverClient.clearCache()` след мутации.
2. Задайте `setCacheEnabled(false)` за крайни точки, които изискват свежи данни.
3. Предайте `AbortSignal` , за да заобиколите кеша за конкретни заявки.

### Грешки при изчакване на външни API

1. Увеличете времето за изчакване в конфигурацията на клиента.
2. Използвайте `externalClient` , който има 15-секундно изчакване.
3. Проверете мрежовата свързаност и разделителната способност на DNS.

### React Query не се извлича повторно

1. Уверете се, че `queryKey` включва всички параметри, които трябва да задействат повторно извличане.
2. Проверете дали `staleTime` не е зададено твърде високо за вашия случай на употреба.
3. Използвайте `queryClient.invalidateQueries` след мутации.
