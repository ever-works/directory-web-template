...
title: Задълбочено гмуркане на крайни точки на API за елементи
sidebar_label: Задълбочено гмуркане на API за елементи
sidebar_position: 65
---

# Задълбочено гмуркане на крайни точки на API за елементи

API за артикули предоставя публични крайни точки за взаимодействие с артикули, включително коментари, гласове, проследяване на изгледи, фирмени асоциации и показатели за ангажираност. Тези крайни точки захранват основните функции, насочени към потребителя, на уебсайта на директорията.

**Директория източник:** `template/app/api/items/`
...
Връща всички коментари за конкретен елемент, включително информация за потребителския профил.

|Собственост|Стойност|
|----------|-------|
|**Метод**|`GET`|
|**Пътека**|`/api/items/{slug}/comments`|
|**Удостоверяване**|Няма (публично)|
|**Източник**|`items/[slug]/comments/route.ts`|
...
Актуализира съдържанието и/или оценката на коментар. Само авторът на коментара може да актуализира своя коментар.

|Собственост|Стойност|
|----------|-------|
|**Метод**|`PUT`|
|**Пътека**|`/api/items/{slug}/comments/{commentId}`|
|**Удостоверяване**|Сесия (собственик на коментар)|
|**Източник**|`items/[slug]/comments/[commentId]/route.ts`|

#### Тяло на заявката

Трябва да се предостави поне едно поле:
...
|-------|------|----------|-------------|
|`content`|`string`|не|1-1000 знака|
|`rating`|`integer`|не| 1-5 |

#### Отговор

**Статус 200** -- Връща актуализирания коментар с потребителска информация и `editedAt` клеймо за време.
...
Връща обобщена статистика за оценка за артикул: средна оценка и общ брой.

|Собственост|Стойност|
|----------|-------|
|**Метод**|`GET`|
|**Пътека**|`/api/items/{slug}/comments/rating`|
|**Удостоверяване**|Няма (публично)|
|**Източник**|`items/[slug]/comments/rating/route.ts`|
...
|Поле|Тип|Описание|
|-------|------|-------------|
|`averageRating`|`number`|Средна оценка (0, ако няма оценки, максимум 5)|
|`totalRatings`|`number`|Общ брой неизтрити коментари с оценки|
...
Крайни точки само за администратори за управление на връзката между елементи и компании.

### Вземете компания за артикули

|Собственост|Стойност|
|----------|-------|
|**Метод**|`GET`|
|**Пътека**|`/api/items/{slug}/company`|
|**Удостоверяване**|Админ|
|**Източник**|`items/[slug]/company/route.ts`|

#### Отговор

**Статус 200** -- Открита фирма.
...
**Статус 200** -- Няма присвоена компания.
...
### Премахване на компанията от артикула

Премахва фирмената асоциация от елемент. Тази операция е идемпотентна.
...
Връща общия брой гласове и статуса на гласуване на текущия потребител (ако е удостоверен).

|Собственост|Стойност|
|----------|-------|
|**Метод**|`GET`|
|**Пътека**|`/api/items/{slug}/votes`|
|**Удостоверяване**|Няма (обществен; потребителският статус изисква сесия)|
|**Източник**|`items/[slug]/votes/route.ts`|
...
|Поле|Тип|Описание|
|-------|------|-------------|
|`count`|`number`|Нетен брой гласове (гласове за - гласове против)|
|`userVote`|`"нагоре" \|"надолу" \|нула`|Глас на потребителя (`null`, ако не е удостоверен или няма глас)|
...
Премахва гласа на текущия потребител от даден елемент.

|Собственост|Стойност|
|----------|-------|
|**Метод**|`DELETE`|
|**Пътека**|`/api/items/{slug}/votes`|
|**Удостоверяване**|Сесия (потребител с клиентски профил)|
|**Източник**|`items/[slug]/votes/route.ts`|
...
Връща пълния запис на гласуване за гласуването на удостоверения потребител за конкретен елемент.

|Собственост|Стойност|
|----------|-------|
|**Метод**|`GET`|
|**Пътека**|`/api/items/{slug}/votes/status`|
|**Удостоверяване**|Сесия (потребител)|
|**Източник**|`items/[slug]/votes/status/route.ts`|

#### Отговор

**Състояние 200** -- Потребителят е гласувал.
...
**Състояние 200** -- Потребителят не е гласувал.
...
### Резултати за популярност (отстраняване на грешки)

Крайна точка за отстраняване на грешки, която връща елементи, сортирани по техния изчислен рейтинг за популярност с подробна разбивка на факторите за оценяване.

|Собственост|Стойност|
|----------|-------|
|**Метод**|`GET`|
|**Пътека**|`/api/items/popularity-scores`|
|**Удостоверяване**|Няма (публично)|
|**Кеширане**|`force-dynamic`|
|**Източник**|`items/popularity-scores/route.ts`|

#### Параметри на заявката

|Параметър|Тип|Задължително|По подразбиране|Описание|
|-----------|------|----------|---------|-------------|
|`limit`|`integer`|не| `20` |Брой артикули за връщане (макс. 100)|
|`locale`|`string`|не|`"en"`|Език за елементи|
...
Резултатът за популярност използва логаритмично мащабиране, за да предотврати доминирането на извънредните стойности:

|Фактор|Тегло|Формула|
|--------|--------|---------|
|Представен тласък| 10000 |Плосък бонус за представени артикули|
|Изгледи| 1000 |`log10(views + 1) * 1000`|
|Гласове| 1200 |`log10(max(votes, 0) + 1) * 1200`|
|Средна оценка| 500 |`avgRating * 500`|
|Любими| 1100 |`log10(favorites + 1) * 1100`|
|Коментари| 1000 |`log10(comments + 1) * 1000`|
|Актуалност|до 1000|Разпадащ се бонус за артикули под 180 дни|

Елементите без данни за ангажираност получават малък евристичен резултат въз основа на качеството на метаданните (брой тагове, дължина на името, присъствие на икона, промоционален код).
