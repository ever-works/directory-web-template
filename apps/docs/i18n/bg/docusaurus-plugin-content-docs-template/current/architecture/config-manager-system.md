...
`lib/config-manager.ts` управлява файла `works.yaml` в директорията `.content/` (клониран от хранилището на данни). Той чете и записва YAML конфигурация и автоматично извършва и изпраща промени в хранилището на Git с помощта на `isomorphic-git`. Това се използва за настройки, които администраторите могат да променят по време на изпълнение (страниране, навигация, горен/долен колонтитул).
