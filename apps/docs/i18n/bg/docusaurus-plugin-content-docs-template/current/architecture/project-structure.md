...
title: Справка за структурата на проекта
sidebar_label: Структура на проекта
sidebar_position: 4
---

# Справка за структурата на проекта

Този документ предоставя пълна справка за структурата на директорията на шаблона Ever Works. Шаблонът е **Turborepo monorepo**, управляван с pnpm работни пространства. Основното приложение е приложение Next.js 16, използващо App Router, TypeScript, Drizzle ORM и Tailwind CSS, разположено в `apps/web/`.

## Преглед от най-високо ниво (Monorepo Root)
...
Основното приложение се намира в `apps/web/`. Всички изходни директории, които преди са били в корена на хранилището, сега се намират тук.
...
## Монорепо инструменти

Шаблонът използва **Turborepo** за оркестрация на изграждането и **pnpm работни пространства** за управление на зависимостите в monorepo.

|Инструмент|Цел|Конфигурация|
|------|---------|---------------|
|Турборепо|Оркестрация на задачи, кеширане, паралелни компилации|`turbo.json`|
|pnpm работни пространства|Управление на зависимости, свързване на работно пространство|`pnpm-workspace.yaml`|
|`packages/tsconfig`|Споделени базови конфигурации на TypeScript|Разширено с всяко приложение|
|`packages/eslint-config`|Споделени предварително зададени плоски конфигурации на ESLint 9|Импортирано от всяко приложение|

### Приложения за работно пространство

- **`apps/web`** -- Основното приложение Next.js 16 (списък с директории, административен панел, API маршрути).
- **`apps/web-e2e`** -- Тестове от край до край на драматург, насочени към `apps/web`. Разделено в собствено работно пространство за изолирани зависимости и независими CI изпълнения.
- **`apps/docs`** -- Сайт с документация на Docusaurus v3 за самия шаблон.
