---
id: user-types
title: Дефиниции на тип потребител и профил
sidebar_label: Типове потребители
sidebar_position: 2
---

# Дефиниции на тип потребител и профил

**Източник:** `lib/types/user.ts` и `lib/types/profile.ts`

Тези модули дефинират типове за администраторски потребители (удостоверяване и управление) и публични потребителски профили (дисплей и портфолио).
...
Потребителски данни само за удостоверяване, съхранявани в таблицата `users`. Това е минималното потребителско представяне, използвано за потоци за удостоверяване.
...
Пълни потребителски данни, които включват информация за профила от `clientProfiles`. Това е пълното потребителско представяне, използвано в таблото за управление на администратора.
...
**Разлика от `AuthUserData`:** В `UserData`, `username`, `name`, `role`, `status` и `created_by` са задължителни (незадължителни).

#### `CreateUserRequest`

Полезен товар за създаване на нов администраторски потребител.
...
Полезен товар за актуализиране на съществуващ потребител. Всички полета не са задължителни.
...
Страниран отговор за потребителски списъци за удостоверяване.
...
Страниран отговор за пълни потребителски списъци.
...
Обвивка на един потребителски отговор.
...
Параметри на заявката за филтриране и страниране на потребителски списъци.
...
Разширени потребителски данни с незадължително поле за преброяване за статистика.
...
Пълна схема за валидиране за създаване на потребител:
...
Частична схема за актуализации (без парола):
...
Въведете guard, който стеснява низа до `UserStatus`:
...
Генерира UUID за нов потребител:
...
Форматира `Date` като YAML-съвместим низ (`YYYY-MM-DD HH:mm`):
