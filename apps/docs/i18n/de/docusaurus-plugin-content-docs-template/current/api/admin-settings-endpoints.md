...
Gibt das gesamte Einstellungsobjekt aus `works.yaml` zurück. Sensible Schlüssel werden maskiert oder ausgeschlossen.
...
Aktualisiert einen einzelnen verschachtelten Konfigurationsschlüssel unter Verwendung von Punkt-Notation. Intern wird `configManager.updateNestedKey(key, value)` aufgerufen, das die Änderung in `works.yaml` schreibt und den In-Memory-Konfigurationscache neu lädt.
...
| `500` | Fehler beim Schreiben in `works.yaml` |
