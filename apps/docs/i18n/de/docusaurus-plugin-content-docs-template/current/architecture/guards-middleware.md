...
title: Wachen und Middleware
sidebar_label: Wachen und Middleware
sidebar_position: 40
---

# Wachen und Middleware

Auf dieser Seite werden zwei wichtige Autorisierungssubsysteme in der Vorlage behandelt: der **Plan Features Guard** für die abonnementbasierte Zugriffskontrolle und die **Permission Check Middleware** für die rollenbasierte Durchsetzung von Berechtigungen.
...
Gibt die numerische Ebene für eine Planzeichenfolge zurück. Gibt `0` für unbekannte Pläne zurück.
...
Überprüft, ob die Planstufe eines Benutzers größer oder gleich der erforderlichen Planstufe ist.
...
Die primäre Zugriffskontrollfunktion. Wertet die Funktionszugriffsregel anhand des Plans des Benutzers aus.

```ts
function canAccessFeature(feature: Feature, userPlan: string): boolean
```

Verarbeitet alle Zugriffsregeltypen: `'all'`, Einzelplan, Planarray und `{ minPlan }` Objekte.

#### `getFeatureLimit(limitName, userPlan): number | null`

Gibt den numerischen Grenzwert für eine Funktion im Plan des Benutzers zurück. Fällt für unbekannte Pläne auf `FREE` Planlimits zurück.
...
Gibt `true` zurück, wenn der Wert das Limit des Plans nicht überschreitet. Gibt `true` für unbegrenzte (`null`) Limits zurück.
...
Gibt die vollständige Liste der Funktionen zurück, auf die der angegebene Plan zugreift.

#### `getMinimumPlanForFeature(feature): PaymentPlan`

Gibt den niedrigsten Plan zurück, der Zugriff auf die angegebene Funktion gewährt.
...
### Reagieren Sie auf den Hook-Helfer

#### `createPlanGuardResult(userPlan): PlanGuardResult`

Erstellt ein serialisierbares Ergebnisobjekt, das für React-Hooks und die clientseitige Verwendung geeignet ist.
...
Prüft, ob der Benutzer über eine bestimmte Berechtigung verfügt.
...
Gibt `true` zurück, wenn der Benutzer über mindestens eine der aufgelisteten Berechtigungen verfügt (ODER-Logik).
...
Gibt `true` nur zurück, wenn der Benutzer über alle aufgelisteten Berechtigungen (UND-Logik) verfügt.
...
Erstellt eine Berechtigungszeichenfolge aus `resource` und `action`, validiert sie mit `isValidPermission` und überprüft dann den Zugriff. Protokolliert eine Warnung für ungültige Berechtigungszeichenfolgen.
...
Gibt alle Berechtigungen des Benutzers zurück, die zu einer bestimmten Ressource gehören (Filterung nach Präfix).
...
Gruppiert die Berechtigungen des Benutzers nach Ressource. Nützlich zum Rendern von Berechtigungsmatrizen in Admin-Benutzeroberflächen.
...
Gibt `true` zurück, wenn es sich bei der Zeichenfolge um eine erkannte Systemberechtigung handelt. Verwendet eine O(1) `Set`-Suche, die aus `PERMISSIONS`-Definitionen erstellt wurde.

#### `parsePermission(permission): object | null`

Teilt eine gültige Berechtigungszeichenfolge in `{ resource, action }` auf. Gibt `null` für ungültige Berechtigungen zurück.
...
|Datei|Beziehung|
|------|-------------|
|`lib/constants.ts`|`PaymentPlan` Enumerationsdefinition|
|`lib/permissions/definitions.ts`|`PERMISSIONS` Karte und `isValidPermission`|
|`lib/types/role.ts`|Rollentypdefinitionen|
