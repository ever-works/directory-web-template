...
id: guards-module
title: Guards-Modul
sidebar_label: Guards-Modul
sidebar_position: 58
---

# Guards-Modul

Das Guards-Modul (`template/lib/guards/`) bietet ein planbasiertes Feature-Zugriffskontrollsystem. Es definiert, welche Abonnementpläne auf welche Funktionen zugreifen können, erzwingt numerische Beschränkungen pro Plan und bietet sowohl serverseitige Schutzfunktionen als auch einen reaktionsfreundlichen Ergebnisgenerator für die clientseitige Verwendung.
...
|Datei|Beschreibung|
|------|-------------|
|`lib/guards/index.ts`|Fassexporte|
|`lib/guards/plan-features.guard.ts`|Vollständige Guard-Implementierung|

## Planhierarchie

Die Pläne sind nach Ebene sortiert, wobei höhere Ebenen alle Funktionen niedrigerer Ebenen umfassen, wenn sie mit `minPlan` konfiguriert werden:
...
Alle verfügbaren Funktionen werden als Konstanten deklariert:
...
Der Datensatz `FEATURE_ACCESS` ordnet jedes Feature seiner Zugriffsregel zu:
...
### Zusammenfassung der Zugriffsregeln

|Funktion|Zugriffsregel|
|---------|------------|
|`submit_product`|`'all'`|
...
Numerische Grenzwerte für Features mit Mengenbeschränkungen. `null` bedeutet unbegrenzt.
...
## Zugriffsprüfungsfunktionen

### `canAccessFeature(feature: Feature, userPlan: string): boolean`

Kernzugriffsprüfung, die die Funktionszugriffsregel auswertet:
...
Gibt den Grenzwert für einen Plan zurück:
...
Überprüft, ob ein Wert innerhalb des Planlimits liegt:
...
Gibt alle Funktionen zurück, auf die ein bestimmter Plan zugreifen kann.

### `getMinimumPlanForFeature(feature: Feature): PaymentPlan`

Gibt den niedrigsten Plan zurück, der auf eine Funktion zugreifen kann.

## Planwachenfabrik

### `createPlanGuard(userPlan: string)`

Erstellt eine Guard-Instanz mit gebundenen Methoden für einen bestimmten Plan:
...
Benutzerdefinierter Fehler, der von `requireFeature` ausgelöst wird:
...
Erstellt ein einfaches Objekt, das für die Verwendung in React-Hooks und -Komponenten geeignet ist:
