...
title: Referenz zur Projektstruktur
sidebar_label: Projektstruktur
sidebar_position: 4
---

# Referenz zur Projektstruktur

Dieses Dokument bietet eine vollständige Referenz der Ever Works-Vorlagenverzeichnisstruktur. Die Vorlage ist ein **Turborepo-Monorepo**, das mit pnpm-Arbeitsbereichen verwaltet wird. Die primäre Anwendung ist eine Next.js 16-App, die App Router, TypeScript, Drizzle ORM und Tailwind CSS verwendet und sich in `apps/web/` befindet.

## Top-Level-Übersicht (Monorepo Root)
...
Die primäre Anwendung befindet sich in `apps/web/`. Alle Quellverzeichnisse, die sich zuvor im Repository-Stammverzeichnis befanden, befinden sich jetzt hier.
...
Die Vorlage verwendet **Turborepo** für die Build-Orchestrierung und **pnpm-Arbeitsbereiche** für die Abhängigkeitsverwaltung im gesamten Monorepo.

|Werkzeug|Zweck|Konfiguration|
|------|---------|---------------|
|Turborepo|Aufgabenorchestrierung, Caching, parallele Builds|`turbo.json`|
|pnpm-Arbeitsbereiche|Abhängigkeitsmanagement, Arbeitsbereichsverknüpfung|`pnpm-workspace.yaml`|
|`packages/tsconfig`|Gemeinsam genutzte TypeScript-Basiskonfigurationen|Erweitert durch jede App|
|`packages/eslint-config`|Gemeinsame Voreinstellungen für flache ESLint 9-Konfigurationen|Von jeder App importiert|

### Workspace-Apps

- **`apps/web`** – Die Hauptanwendung von Next.js 16 (Verzeichnisliste, Admin-Panel, API-Routen).
- **`apps/web-e2e`** – Dramatiker-End-to-End-Tests für `apps/web`. Getrennt in einen eigenen Arbeitsbereich für isolierte Abhängigkeiten und unabhängige CI-Ausführungen.
- **`apps/docs`** – Docusaurus v3-Dokumentationsseite für die Vorlage selbst.
