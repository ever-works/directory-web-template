...
Standorteinstellungen werden im Abschnitt `settings.location` der `works.yml` Ihres Inhalts-Repositorys definiert:
...
Die rohe Form, die aus `works.yml` gelesen wird, definiert in `lib/types/location.ts`:
