...
title: Kategorie- und Standortindexabfragen – tiefer Einblick
sidebar_label: Kategorieabfragen im Detail
sidebar_position: 66
---

# Kategorie- und Standortindexabfragen – tiefer Einblick

Umfassende Referenz für alle Standortindex- und Artikelprüfungsabfragefunktionen, einschließlich Geodatensuche, Begrenzungsrahmenabfragen, Stapelindizierung, Prüfprotokollierung und Artikelverlaufsverfolgung.

## Übersicht

Die Kategorie-/Standort-Abfrageschicht verwaltet zwei verwandte Indexierungssysteme:

- **`location-index.queries.ts`** – Georäumlicher Index für schnelle standortbasierte Elementabfragen (Stadt, Land, Begrenzungsrahmen, entfernte Elemente) mit Batch-Upsert und Metadatenverfolgung
- **`item-audit.queries.ts`** – Artikel-Audit-Trail zur Verfolgung aller Änderungen, Statusübergänge und Administratoraktionen an Artikeln

Die Standortindextabelle dient als **Sekundärindex** für schnelle Geoabfragen. Die Quelle der Wahrheit für Artikelstandortdaten bleibt in YAML-Dateien im Git-basierten CMS.
...
Batch fügt mehrere Standorteinträge in 100er-Blöcken ein, um Beschränkungen der Abfragegröße zu vermeiden.
...
|-----------|--------------------------------|----------|------------------------|
|`entries`|`UpsertLocationIndexParams[]`|Ja|Array von Standortdaten|

**Rückgaben:** `number` – Anzahl der verarbeiteten Einträge

**SQL-Muster:** Verwendet die Syntax `excluded.*` für Massen-Upsert:
...
**Leistungshinweise:**
- Verarbeitet in Stapeln von 100, um die Größenbeschränkungen für PostgreSQL-Abfragen einzuhalten
- Verwendet `excluded.*` Referenzen für eine effiziente Massenkonfliktlösung
- Setzt `indexedAt` auf den aktuellen Zeitstempel für jeden Eintrag
...
Entfernt mehrere Elemente aus dem Standortindex.
...
Ruft einen Standorteintrag nach Artikel-Slug ab.
...
Fragt Elemente innerhalb eines geografischen Begrenzungsrahmens ab. Nützlich für die anfängliche Radiusfilterung vor der präzisen Entfernungsberechnung.
...
|Parameter|Typ|Erforderlich|Beschreibung|
|--------------|----------|----------|----------------------------|
|`minLat`|`number`|Nein|Minimaler Breitengrad|
|`maxLat`|`number`|Nein|Maximaler Breitengrad|
|`minLng`|`number`|Nein|Minimaler Längengrad|
|`maxLng`|`number`|Nein|Maximaler Längengrad|
|`limit`|`number`|Nein|Maximale Ergebnisse|
|`offset`|`number`|Nein|Paginierungsversatz|
...
Fragt Elemente nach Ländernamen mithilfe der indizierten normalisierten Spalte ab.
...
Ruft alle indizierten Element-Slugs ab.
...
Ruft nur Remote-Standorteinträge mit minimalen Feldern (Slug, Stadt, Land) ab. Optimiert für Radiusabfragen.

```typescript
async function getRemoteLocationEntries(): Promise<RemoteLocationEntry[]>
```

### Abfragen nach eindeutigen Werten

#### `getDistinctCities`

Ruft alle eindeutigen Städtenamen ab, dedupliziert durch normalisierte Form.
...
**Design-Hinweis:** Gruppiert nach normalisierter Spalte, gibt aber den `MIN()` des Originals zurück (was großgeschriebene Formen bevorzugt) und liefert saubere Anzeigewerte.
...
Ruft Statistiken zum Standortindex ab.

```typescript
async function getLocationIndexStats(): Promise<LocationIndexStats>
```

**Rückgaben:** `{ totalIndexed, lastIndexedAt, citiesCount, countriesCount, remoteCount }`
...
Ruft die Standortindex-Metadaten ab (letzte Neuerstellungszeit, Dauer, Elementanzahl).
...
Ruft den paginierten Prüfverlauf für ein Element mit optionaler Aktionstypfilterung ab.
...
Ruft nach Aktionstyp gefilterte Überwachungsprotokolle ab.
...
Ruft Überwachungsprotokollstatistiken für ein Element ab, gruppiert nach Aktionstyp.
