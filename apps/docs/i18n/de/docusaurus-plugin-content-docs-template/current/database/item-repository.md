---
id: item-repository
title: Artikel-Repository
sidebar_label: Artikel-Repository
sidebar_position: 16
---

# Artikel-Repository

Die Klasse `ItemRepository` stellt die primäre Datenzugriffsebene für die Verwaltung von Elementen (Einträge/Einreichungen) in der Vorlage bereit. Es delegiert die Speicherung an einen Git-gestützten Dienst und fügt darüber hinaus Validierung, Filterung, Paginierung, Audit-Protokollierung und Soft-Delete-Unterstützung hinzu.

**Quelldatei:** `template/lib/repositories/item.repository.ts`
...
|Importieren|Zweck|
|--------|---------|
|`ItemData`, `CreateItemRequest`, `UpdateItemRequest`, `ReviewRequest`, `ItemListOptions`|Typdefinitionen von `@/lib/types/item`|
|`createItemGitService` / `ItemGitService`|Git-gestützter Speicherdienst|
|`getContentPath`|Löst das lokale Inhaltsverzeichnis im Vergleich zum Vercel-Inhaltsverzeichnis auf|
|`coreConfig`|Zentralisierter Konfigurationsdienst|
|`itemAuditService` / `AuditUser`|Audit-Trail-Protokollierung|
...
Gibt alle Elemente zurück, die den bereitgestellten Filtern entsprechen. Wendet die folgende Filterkette der Reihe nach an:

1. **Status** – genaue Übereinstimmung bei `item.status`
2. **Kategorien** – ODER-Logik; Das Element muss mindestens eine der angeforderten Kategorien enthalten
3. **Tags** – ODER-Logik; Das Element muss mindestens eines der angeforderten Tags enthalten
4. **submittedBy** – genaue Übereinstimmung auf `item.submitted_by`
5. **Suche** – Teilzeichenfolgenübereinstimmung ohne Berücksichtigung der Groß-/Kleinschreibung für `item.name` oder `item.description`
...
|-----------|------|---------|-------------|
|`options.status`|`string`| -- |Filtern Sie nach Artikelstatus|
|`options.categories`|`string[]`| -- |Nach Kategorie Schnecken filtern (ODER)|
|`options.tags`|`string[]`| -- |Nach Tag-Namen filtern (ODER)|
|`options.submittedBy`|`string`| -- |Filtern Sie nach der Benutzer-ID des Absenders|
|`options.search`|`string`| -- |Freitextsuche|
|`options.includeDeleted`|`boolean`|`false`|Vorläufig gelöschte Elemente einbeziehen|
...
Sucht ein einzelnes Element anhand seiner eindeutigen ID.
...
Stapelsuche mehrerer Artikel anhand ihrer Slugs. Gibt ein leeres Array zurück, wenn die Eingabe leer ist.
...
Aktualisiert ein vorhandenes Element. Erfasst den vorherigen Status für die Audit-Diff-Protokollierung.
...
Überprüft einen Artikel (genehmigen oder ablehnen). Überprüft, ob `reviewData.status` entweder `"approved"` oder `"rejected"` ist.
...
Markiert ein Element als gelöscht (setzt `deleted_at`), ohne die Datei zu entfernen.
...
Gibt `true` zurück, wenn ein Element (einschließlich gelöschter) die angegebene ID aufweist.

### `checkDuplicateSlug(slug): Promise<boolean>`

Gibt `true` zurück, wenn ein Element (einschließlich gelöscht) den angegebenen Slug teilt.

### `getStats(options?): Promise<StatsObject>`

Gibt die Statusanzahl zurück, gefiltert nach den optionalen Einschränkungen `submittedBy`, `search`, `categories` und `tags`.
...
Elemente mit einem `deleted_at` Zeitstempel werden getrennt von aktiven Elementen gezählt.
