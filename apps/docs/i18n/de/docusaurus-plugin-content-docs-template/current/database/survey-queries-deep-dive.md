...
title: Umfrageanfragen im Detail
sidebar_label: Umfrageanfragen im Detail
sidebar_position: 68
---

# Umfrageanfragen im Detail

Umfassende Referenz für alle umfragebezogenen Datenbankabfragefunktionen, einschließlich Umfrage-CRUD, Antwortverwaltung, Paginierung, Filterung und Antwortzählung.

## Übersicht

Die Umfrage-Abfrageebene verwaltet Umfragen und ihre Antworten:

- **`survey.queries.ts`** – Vollständiger Umfrage-CRUD mit Soft-Deletion, paginierter Auflistung mit Filtern, Antwortanzahl und Abschlussstatus-JOINs, Umfrageantwort-CRUD mit Datumsbereichsfilterung und Antwortzählung

Umfragen unterstützen zwei Typen: `global` (siteweit) und `item` (mit einem bestimmten Element verknüpft). Antworten sind sowohl mit einer Umfrage als auch optional mit einem Element verknüpft, sodass Feedback pro Element erfasst werden kann.
...
Ruft eine Umfrage über ihren URL-Slug ab, mit optionalem Ausschluss einer bestimmten Umfrage-ID (nützlich für die Eindeutigkeitsüberprüfung bei Aktualisierungen).
...
|------------|----------|----------|-------------------------------------------|
|`slug`|`string`|Ja|Umfrageschnecke|
|`ignoreId`|`string`|Nein|Umfrage-ID, die von der Suche ausgeschlossen werden soll|

**Rücksendungen:** Umfrage oder `null`, falls nicht gefunden
...
**Rückgabe:** Der erstellte Umfragedatensatz (über `RETURNING`)
...
Soft löscht eine Umfrage, indem `deletedAt` auf den aktuellen Zeitstempel gesetzt wird.
...
**Hinweis:** Alle Leseabfragen werden nach `deleted_at IS NULL` gefiltert, sodass vorläufig gelöschte Umfragen effektiv vor allen Abruffunktionen ausgeblendet werden.
...
Ruft paginierte Umfrageantworten mit optionaler Filterung nach Element, Benutzer und Datumsbereich ab.
...
|-------------|----------|----------|---------|-----------------------------------|
|`surveyId`|`string`|Ja| --      |Umfrage-ID, für die Antworten eingeholt werden sollen|
|`itemId`|`string`|Nein| --      |Nach Artikel filtern|
|`userId`|`string`|Nein| --      |Nach Benutzer filtern|
|`startDate`|`string`|Nein| --      |Filtern Sie Antworten nach diesem Datum|
|`endDate`|`string`|Nein| --      |Filtern Sie Antworten vor diesem Datum|
|`page`|`number`|Nein| `1`     |Seitenzahl|
|`limit`|`number`|Nein| `10`    |Ergebnisse pro Seite|

**Rückgabe:** Paginierte Antwortliste
...
**Leistungshinweise:** Daten- und Zählabfragen werden parallel über `Promise.all` ausgeführt.
...
Ruft die Gesamtzahl der Antworten für eine Umfrage ab.
...
**Rückgabe:** Anzahl der Antworten als Zahl
