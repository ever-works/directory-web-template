...
title: "Benutzerschema Deep Dive"
sidebar_label: "Benutzerschema"
sidebar_position: 51
---

# Benutzerschema Deep Dive

## Übersicht

Das Benutzermodul ist die zentrale Identitätsebene der Ever Works-Vorlage. Es umfasst die Basistabelle `users` (NextAuth-kompatibel), `client_profiles` für erweiterte Profildaten, authentifizierungsbezogene Tabellen (`accounts`, `sessions`, `authenticators`, `verificationTokens`, `passwordResetTokens`) und das RBAC-System (`roles`, `permissions`, `rolePermissions`, `userRoles`).
...
Erweitertes Profil für Anwendungsbenutzer (Clients). Speichert persönliche Daten, Kontoeinstellungen, Moderationsstatus und Standortpräferenzen.
...
|`email`|`email`|`text`|Nein| - | - |
|`name`|`name`|`text`|Nein| - | - |
|`displayName`|`display_name`|`text`|Ja| - | - |
|`username`|`username`|`text`|Ja| - |Einzigartig|
|`bio`|`bio`|`text`|Ja| - | - |
...
|`defaultLatitude`|`default_latitude`|`doublePrecision`|Ja| - |„Near Me“-Fallback|
|`defaultLongitude`|`default_longitude`|`doublePrecision`|Ja| - |„Near Me“-Fallback|
...
|`notes`|`notes`|`text`|Ja| - |Admin-Notizen|
|`tags`|`tags`|`text`|Ja| - | - |
|`warningCount`|`warning_count`|`integer`|Ja| `0` |Moderation|
|`suspendedAt`|`suspended_at`|`timestamp (tz)`|Ja| - |Moderation|
|`bannedAt`|`banned_at`|`timestamp (tz)`|Ja| - |Moderation|
...
|Name|Spalten|Typ|
|---|---|---|
|`client_profile_user_id_unique_idx`|`userId`|Einzigartig|
|`client_profile_email_idx`|`email`|B-Baum|
|`client_profile_status_idx`|`status`|B-Baum|
|`client_profile_plan_idx`|`plan`|B-Baum|
|`client_profile_account_type_idx`|`accountType`|B-Baum|
|`client_profile_username_idx`|`username`|B-Baum|
|`client_profile_created_at_idx`|`createdAt`|B-Baum|
|`client_profiles_username_unique`|`username`|Einzigartig|
...
Aktive Benutzersitzungen für NextAuth.

### Spalten

|Spalte|DB-Name|Typ|Nullbar|Standard|Einschränkungen|
|---|---|---|---|---|---|
|`sessionToken`|`sessionToken`|`text`|Nein| - |Primärschlüssel|
|`userId`|`userId`|`text`|Nein| - |FK -> `users.id` (KASKADE)|
|`expires`|`expires`|`timestamp`|Nein| - | - |
...
|Spalte|DB-Name|Typ|Nullbar|Standard|Einschränkungen|
|---|---|---|---|---|---|
|`id`|`id`|`text`|Nein| - |Primärschlüssel|
|`name`|`name`|`text`|Nein| - |Einzigartig|
|`description`|`description`|`text`|Ja| - | - |
|`isAdmin`|`is_admin`|`boolean`|Nein|`false`| - |
|`status`|`status`|`text (enum)`|Ja|`'active'`|`active`, `inactive`|
|`created_by`|`created_by`|`text`|Ja|`'system'`| - |
|`createdAt`|`created_at`|`timestamp`|Nein|`now()`| - |
|`updatedAt`|`updated_at`|`timestamp`|Nein|`now()`| - |
|`deletedAt`|`deleted_at`|`timestamp`|Ja| - |Vorläufiges Löschen|

**Indizes:** `roles_status_idx`, `roles_is_admin_idx`, `roles_created_at_idx`
...
|---|---|---|---|---|---|
|`id`|`id`|`text`|Nein|`crypto.randomUUID()`|Primärschlüssel|
|`key`|`key`|`text`|Nein| - |Einzigartig|
|`description`|`description`|`text`|Ja| - | - |
|`createdAt`|`created_at`|`timestamp`|Nein|`now()`| - |
|`updatedAt`|`updated_at`|`timestamp`|Nein|`now()`| - |

### Tabelle: `role_permissions` (Kreuzung)

|Spalte|DB-Name|Typ|Nullbar|Standard|Einschränkungen|
|---|---|---|---|---|---|
|`roleId`|`role_id`|`text`|Nein| - |FK -> `roles.id` (KASKADE)|
|`permissionId`|`permission_id`|`text`|Nein| - |FK -> `permissions.id` (KASKADE)|
|`createdAt`|`created_at`|`timestamp`|Nein|`now()`| - |

**Primärschlüssel:** Zusammengesetztes `(roleId, permissionId)`
**Indizes:** `role_permissions_role_idx`, `role_permissions_permission_idx`, `role_permissions_created_at_idx`

### Tabelle: `user_roles` (Kreuzung)

|Spalte|DB-Name|Typ|Nullbar|Standard|Einschränkungen|
|---|---|---|---|---|---|
|`userId`|`user_id`|`text`|Nein| - |FK -> `users.id` (KASKADE)|
|`roleId`|`role_id`|`text`|Nein| - |FK -> `roles.id` (KASKADE)|
|`createdAt`|`created_at`|`timestamp`|Nein|`now()`| - |

**Primärschlüssel:** Zusammengesetztes `(userId, roleId)`
**Indizes:** `user_roles_user_idx`, `user_roles_role_idx`, `user_roles_created_at_idx`
...
### Tabelle: `verificationTokens`

|Spalte|Typ|Einschränkungen|
|---|---|---|
|`identifier`|`text`|Verbund-PK-Teil|
|`email`|`text`|NICHT NULL|
|`token`|`text`|Verbund-PK-Teil|
|`expires`|`timestamp`|NICHT NULL|

**Primärschlüssel:** Zusammengesetztes `(identifier, token)`

### Tabelle: `passwordResetTokens`

|Spalte|Typ|Einschränkungen|
|---|---|---|
|`id`|`text`|Primärschlüssel (`crypto.randomUUID()`)|
|`email`|`text`|NICHT NULL|
|`token`|`text`|NICHT NULL, Eindeutig|
|`expires`|`timestamp`|NICHT NULL|
...
### Protokollieren Sie eine Aktivität
