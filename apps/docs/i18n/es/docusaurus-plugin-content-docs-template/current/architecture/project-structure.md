...
title: Referencia de la estructura del proyecto
...
Este documento proporciona una referencia completa de la estructura del directorio de plantillas de Ever Works. La plantilla es un **Turborepo monorepo** administrado con espacios de trabajo pnpm. La aplicación principal es una aplicación Next.js 16 que utiliza App Router, TypeScript, Drizzle ORM y Tailwind CSS, ubicada en `apps/web/`.

## Descripción general de nivel superior (raíz de Monorepo)
...
La aplicación principal se encuentra en `apps/web/`. Todos los directorios de origen que anteriormente estaban en la raíz del repositorio ahora residen aquí.
...
## Herramientas Monorepo

La plantilla utiliza **Turborepo** para la orquestación de compilaciones y **espacios de trabajo pnpm** para la gestión de dependencias en todo monorepo.

|Herramienta|Propósito|Configuración|
|------|---------|---------------|
|Turborepo|Orquestación de tareas, almacenamiento en caché, compilaciones paralelas|`turbo.json`|
|espacios de trabajo pnpm|Gestión de dependencias, vinculación de espacios de trabajo.|`pnpm-workspace.yaml`|
|`packages/tsconfig`|Configuraciones básicas compartidas de TypeScript|Extendido por cada aplicación|
|`packages/eslint-config`|Ajustes preestablecidos de configuración plana de ESLint 9 compartidos|Importado por cada aplicación|

### Aplicaciones de espacio de trabajo

- **`apps/web`** -- La aplicación principal Next.js 16 (listado de directorios, panel de administración, rutas API).
- **`apps/web-e2e`** -- Pruebas integrales de dramaturgos dirigidas a `apps/web`. Separado en su propio espacio de trabajo para dependencias aisladas y ejecuciones de CI independientes.
- **`apps/docs`** -- Sitio de documentación de Docusaurus v3 para la propia plantilla.
