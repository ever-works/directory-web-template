...
sidebar_label: Esquemas de validación
sidebar_position: 39
---

# Esquemas de validación

La plantilla utiliza Zod para la validación del tiempo de ejecución en rutas API, acciones del servidor y envíos de formularios. Los esquemas están organizados por dominio en `lib/validations/` y hacen referencia a ellos mediante código tanto del lado del servidor como del lado del cliente.
...
## Esquemas de autenticación (`auth.ts`)

### Esquema de contraseña

Un esquema compartido que impone requisitos de contraseñas seguras:
...
## Esquemas de artículos (`item.ts`)

### Esquema de ubicación

Valida los datos de ubicación geográfica de los artículos. Todos los campos son opcionales ya que el rigor está controlado por la configuración del sitio:
...
## Esquemas de elementos del cliente (`client-item.ts`)

### Crear artículo

Esquema para elementos enviados por el cliente con campos básicos obligatorios:
...
Solo permite campos que los clientes pueden modificar (todos opcionales):
...
### Consulta de lista de artículos

Valida y transforma parámetros de consulta para listas de elementos paginados:
...
## Esquemas de empresa (`company.ts`)
...
Idéntico a `createCompanySchema` pero con un campo `id` y todos los demás campos son opcionales:
...
### Asociación de empresas de artículos
...
## Esquemas de ubicación de usuario (`user-location.ts`)
...
Valida la ubicación del perfil de usuario con un refinamiento entre campos que garantiza que la latitud y la longitud se proporcionen juntas:
...
## Esquemas de anuncios de patrocinadores (`sponsor-ad.ts`)

### Enumeraciones de estado e intervalo
...
### Actualizar anuncio del patrocinador (administrador)
...
### Consultar anuncios de patrocinadores
...
## Esquemas del panel (`client-dashboard.ts`)
...
### En forma de gancho de reacción
...
- `lib/validations/auth.ts` - Validación de contraseña
- `lib/validations/item.ts` - Esquema de ubicación del artículo
- `lib/validations/client-item.ts` - Esquemas de elementos de cara al cliente
- `lib/validations/client-dashboard.ts` - Esquemas de consulta del panel
- `lib/validations/company.ts` - Esquemas de empresa y de artículo-empresa
- `lib/validations/user-location.ts` - Esquemas de ubicación de usuarios
- `lib/validations/sponsor-ad.ts` - Esquemas del ciclo de vida de los anuncios del patrocinador
- `lib/types/item.ts` - `ITEM_VALIDATION` constantes utilizadas por los esquemas
