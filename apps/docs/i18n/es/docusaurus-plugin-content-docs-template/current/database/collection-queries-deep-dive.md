...
title: Consultas de colección (empresa) Análisis profundo
sidebar_label: Consultas de colección Análisis profundo
sidebar_position: 65
---

# Consultas de colección (empresa) Análisis profundo

Referencia completa para todas las operaciones CRUD de empresa/colección, asociaciones de artículo-empresa, funciones de búsqueda, paginación y consulta de estadísticas.

## Descripción general

La capa de consulta de colección gestiona empresas y sus asociaciones con elementos:

- **`company.queries.ts`** -- CRUD completo de la empresa, búsqueda con paginación, vínculo entre artículo y empresa (relación 1:1), estadísticas y listado de artículos por empresa

Las empresas en este sistema representan las organizaciones detrás de los artículos listados. Cada artículo puede pertenecer como máximo a una empresa (aplicada por una restricción única en `itemSlug`).
...
Obtiene una empresa por dominio utilizando coincidencias que no distinguen entre mayúsculas y minúsculas.
...
Actualiza los campos de la empresa. Normaliza `domain` y `slug` si se proporcionan.
...
**Nota:** Elimina los valores `undefined` de la carga útil de actualización para evitar sobrescribir campos con valores nulos.
...
Enumera empresas con paginación, búsqueda, filtrado y clasificación.
...
**Comportamiento de búsqueda:** Utiliza `ILIKE` en los campos `name` y `domain` con el escape adecuado.

**Datos adicionales:** Devuelve `activeCount` y `inactiveCount` globales independientemente de los filtros aplicados.
...
Enumera las empresas con el recuento de artículos asociados a cada una, lo que admite la clasificación por recuento de artículos.
...
Alias compatible con versiones anteriores para `linkItemToCompany`. Devuelve sólo el objeto de asociación.
...
Elimina la asociación artículo-compañía. Idempotente: desvincular un mapeo inexistente no es una operación.
...
**Devoluciones:**
- `success`: Siempre `true` (idempotente)
- `deleted`: `true` si realmente se eliminó una asociación
...
Obtiene la empresa asociada a un artículo.
...
Comprueba si un artículo tiene una empresa asignada.
...
Alias compatible con versiones anteriores para `listItemsByCompany`.
