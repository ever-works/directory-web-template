...
Le système prend en charge deux fournisseurs d'e-mail configurés via `works.yml` et les variables d'environnement :
...
Le fournisseur est sélectionné en fonction du champ `mail.provider` dans `works.yml`. La configuration des e-mails est construite dynamiquement à partir de la configuration de l'application en utilisant `createEmailConfig()`.
