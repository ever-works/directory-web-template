...
title: Annotations fanfaronnes
sidebar_label: Annotations fanfaronnes
sidebar_position: 36
---

# Annotations fanfaronnes

Le modèle comprend un module utilitaire permettant de générer des annotations JSDoc OpenAPI/Swagger standardisées pour les routes de l'API Next.js App Router. Ce système évite la duplication des schémas et paramètres de réponse communs.
...
Génère une chaîne d'annotation JSDoc Swagger complète à partir d'un objet de configuration :
...
Un raccourci pour les routes réservées aux administrateurs qui ajoute automatiquement la sécurité `sessionAuth` :
...
L'objet `CommonAnnotations` fournit des modèles et des paramètres de réponse réutilisables :
...
Chaque réponse comprend un exemple de corps :
...
Les annotations sont placées sous forme de commentaires JSDoc au-dessus des exportations du gestionnaire de route. Le générateur Swagger les traite pendant la construction pour produire la spécification OpenAPI :
...
Combinez des modèles courants avec une configuration spécifique à un itinéraire :
...
- `lib/swagger/annotations.ts` - Types d'annotations, générateurs et modèles courants
- `app/api/` - Gestionnaires de routes API qui utilisent les annotations
