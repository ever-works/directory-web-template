...
sidebar_label: "Système de thèmes"
sidebar_position: 20
---

# Système de thèmes

Le modèle fournit un système multithème avec quatre thèmes intégrés. Les thèmes contrôlent les couleurs, les variables CSS, les utilitaires Tailwind et incluent des composants d'aperçu et des métadonnées pour les interfaces utilisateur de sélection de thème.
...
|Fichier|Objectif|
|------|---------|
|`lib/themes.tsx`|Définitions de thème, métadonnées et composants d'aperçu|
|`lib/theme-color-manager.ts`|Configurations étendues, application DOM, génération CSS|
|`lib/theme-utils.ts`|Utilitaires de classe Tailwind, préréglages, fonctions d'assistance|
|`components/context/LayoutThemeContext`|Contexte de réaction pour l'état du thème (référencé)|

## Thèmes disponibles

|Clé du thème|Étiquette|Primaire|Secondaire|Descriptif|
|-----------|-------|---------|-----------|-------------|
|`everworks`|Par défaut|`#3d70ef`|`#00c853`|Moderne et professionnel avec du bleu et du vert|
|`corporate`|Entreprise|`#00c853`|`#e74c3c`|Entreprise professionnelle avec du vert et du rouge|
|`material`|Matériel|`#673ab7`|`#ff9800`|Google Material Design avec du violet et de l'orange|
|`funny`|Drôle|`#ff4081`|`#ffeb3b`|Ludique et vibrant avec du rose et du jaune|

## Configuration du thème

Chaque thème définit sept emplacements de couleurs :
...
Le `EXTENDED_THEME_CONFIGS` dans `theme-color-manager.ts` fournit les définitions de couleurs complètes :
...
Le module `themes.tsx` fournit des métadonnées d'affichage et des composants de prévisualisation :
...
Chaque thème a un petit aperçu visuel rendu sous la forme d'un style `div` :
...
### Fonctions de requête de métadonnées
...
Lorsqu'un thème est appliqué, le gestionnaire de couleurs définit les propriétés personnalisées CSS sur `document.documentElement` :
...
Pour chaque thème, les variables CSS suivantes sont créées :

|Modèle variable|Compter|Exemple|
|-----------------|-------|---------|
...
## Classes d’utilitaires Tailwind

Combinaisons de classes prédéfinies pour une utilisation cohérente du thème :
...
### Classes de mise en page réactives
...
## Bâtiment de classe axé sur le thème

La fonction `buildThemeClasses` combine des classes de base, thématiques et conditionnelles :
...
## Préréglages de couleurs du thème

Accès rapide aux couleurs primaires/secondaires du thème :
...
## Référence des couleurs du vent arrière

Le module `theme-utils.ts` exporte également l'ensemble complet des valeurs de couleur CSS Tailwind sous la forme d'un objet `tailwindColors` couvrant les 22 familles de couleurs (ardoise à rose) avec des nuances de 50 à 950, plus une carte `opacities` de 5 % à 95 %.
