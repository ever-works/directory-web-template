...
title: "Catégories et sociétés Schéma approfondi"
sidebar_label: "Schéma des catégories"
sidebar_position: 52
---

# Catégories et sociétés Schéma approfondi

## Aperçu

Dans le modèle Ever Works, les **catégories sont définies dans le CMS basé sur Git** (référentiel de contenu), et non dans la base de données. Il n’existe pas de table de base de données `categories`. Cependant, la base de données fournit une infrastructure permettant de relier les éléments aux entreprises et de suivre les hiérarchies des entreprises, ce qui répond à un objectif organisationnel similaire.

Cette page documente la table `companies`, la table de jonction `items_companies` et la manière dont les références de catégorie/société apparaissent dans le schéma.
...
## Tableau : `items_companies`

Table de jonction reliant les fiches d'articles aux enregistrements de l'entreprise. Chaque slug d'article ne peut être associé qu'à une seule société (contrainte unique sur `item_slug`).

### Colonnes

|Colonne|Nom de la base de données|Tapez|Nullable|Par défaut|Contraintes|
|---|---|---|---|---|---|
|`itemSlug`|`item_slug`|`text`|Non| - |Unique|
|`companyId`|`company_id`|`text`|Non| - |FK -> `companies.id` (CASCADE)|
...
|Nom|Colonnes|Tapez|
|---|---|---|
|`items_companies_company_id_idx`|`companyId`|Arbre B|
...
### Obtenez toutes les entreprises actives
...
### Rechercher une entreprise par domaine
...
### Obtenir des articles pour une entreprise
...
### Lier un article à une entreprise
