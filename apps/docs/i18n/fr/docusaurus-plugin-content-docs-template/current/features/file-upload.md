...
sidebar_label: Téléchargement de fichiers
sidebar_position: 23
---
...
1. User clicks the **Image** button in the editor toolbar or presses `Ctrl+Shift+I`
2. The `useImageUpload` hook inserts an `imageUpload` node into the editor
3. The upload node component renders a file picker or drop zone
4. On file selection, the file is uploaded via `serverClient.upload()`
5. On success, the upload node is replaced with a regular image node referencing the uploaded URL
6. On failure, an error message is displayed within the node
