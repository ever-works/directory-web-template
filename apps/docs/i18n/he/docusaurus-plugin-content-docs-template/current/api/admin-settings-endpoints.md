...
ממשק ה-API של הגדרות הניהול מספק נקודות קצה לקריאה ושינוי של תצורת האתר המאוחסנת ב-`works.yaml`. זה כולל הגדרות כלליות וסטטוס ספק המפה. כל נקודות הקצה דורשות אימות מנהל.
...
מאחזר את הקטע `settings` המלא מהקובץ `works.yaml` של האתר.
...
הצורה המדויקת של האובייקט `settings` תלויה בתצורת `works.yaml` של האתר. נקודת הקצה מחזירה את כל מה שמאוחסן תחת המקש `settings`.
...
מעדכן ערך הגדרה יחיד בקטע `settings` של `works.yaml`. המפתח עובר טווח אוטומטי למרחב השמות `settings` (למשל, מתן עדכוני מפתח `"theme"` `settings.theme` בקובץ התצורה).
...
העדכון נמשך באמצעות `configManager.updateNestedKey()`, אשר משנה את הקובץ `works.yaml` הבסיסי. המפתח מקבל קידומת אוטומטית של `settings.` לפני שהוא מועבר למנהל התצורה.
...
- **אחסון:** ההגדרות מאוחסנות בקובץ תצורה של YAML (`works.yaml`)
