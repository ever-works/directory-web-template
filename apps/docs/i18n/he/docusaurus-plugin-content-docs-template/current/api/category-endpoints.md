...
title: "נקודות קצה של קטגוריה API"
sidebar_label: "קטגוריות"
sidebar_position: 10
---

# נקודות קצה של קטגוריה API

ה-API של Categories מספק נקודת קצה ציבורית קלה לבדיקה אם קיימות קטגוריות במערכת. קטגוריות נגזרות משכבת ​​התוכן (CMS מבוסס Git) ולא ממסד נתונים, מה שהופך את נקודת הקצה הזו לזמינה גם ללא חיבור למסד נתונים.

**קובץ מקור:** `template/app/api/categories/exists/route.ts`

## סיכום נקודות קצה

|שיטה|נתיב|Auth|תיאור|
|--------|------|------|-------------|
|קבל|`/api/categories/exists`|אין|בדוק אם קיימות קטגוריות|
