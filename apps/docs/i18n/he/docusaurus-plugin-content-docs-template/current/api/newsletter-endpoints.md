...
המערכת תומכת בשני ספקי דוא"ל המוגדרים באמצעות `works.yaml` ומשתני סביבה:
...
הספק נבחר על סמך השדה `mail.provider` ב-`works.yaml`. תצורת דוא"ל נבנית באופן דינמי מתצורת האפליקציה באמצעות `createEmailConfig()`.
