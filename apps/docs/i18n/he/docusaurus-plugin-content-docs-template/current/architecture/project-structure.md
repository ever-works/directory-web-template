...
title: התייחסות למבנה הפרויקט
sidebar_label: מבנה הפרויקט
sidebar_position: 4
---

# התייחסות למבנה הפרויקט

מסמך זה מספק התייחסות מלאה למבנה ספריות התבנית של Ever Works. התבנית היא **Turborepo monorepo** המנוהל עם סביבות עבודה של pnpm. האפליקציה העיקרית היא אפליקציית Next.js 16 המשתמשת בנתב האפליקציות, TypeScript, Drizzle ORM ו-Tailwind CSS, הממוקמת ב-`apps/web/`.

## סקירה ברמה העליונה (שורש מונורפו)
...
### apps/web/ -- יישום Next.js

האפליקציה הראשית נמצאת ב-`apps/web/`. כל ספריות המקור שהיו בעבר בשורש המאגר שוכנות כעת כאן.
...
התבנית משתמשת ב-**Turborepo** לתזמור בנייה וב-**סביבות עבודה של pnpm** לניהול תלות ברחבי המונורפו.

|כלי|מטרה|תצורה|
|------|---------|---------------|
|טורבורפו|תזמור משימות, שמירה במטמון, בנייה מקבילה|`turbo.json`|
|pnpm סביבות עבודה|ניהול תלות, קישור סביבות עבודה|`pnpm-workspace.yaml`|
|`packages/tsconfig`|תצורות בסיס משותפות של TypeScript|מורחב על ידי כל אפליקציה|
|`packages/eslint-config`|הגדרות קבועות מראש של תצורה שטוחות של ESLint 9 משותפות|מיובאים על ידי כל אפליקציה|

### אפליקציות סביבת עבודה

- **`apps/web`** -- היישום הראשי של Next.js 16 (רשימת ספריות, פאנל ניהול, מסלולי API).
- **`apps/web-e2e`** -- מבחני מחזאי מקצה לקצה המכוונים ל-`apps/web`. מופרד לתוך סביבת עבודה משלו עבור תלות מבודדת והרצות CI עצמאיות.
- **`apps/docs`** -- אתר תיעוד Docusaurus v3 עבור התבנית עצמה.
