...
title: סכימות אימות
sidebar_label: סכימות אימות
sidebar_position: 39
---

# סכימות אימות

התבנית משתמשת ב-Zod לאימות זמן ריצה בין מסלולי API, פעולות שרת והגשת טפסים. סכימות מאורגנות לפי תחום ב-`lib/validations/` ומתייחסות הן לקוד בצד השרת והן בקוד הלקוח.
...
## סכימות אימות (`auth.ts`)

### סכמת סיסמאות

סכימה משותפת האוכפת דרישות סיסמה חזקה:
...
## סכימות פריטים (`item.ts`)

### סכימת מיקום

מאמת נתוני מיקום גיאוגרפי עבור פריטים. כל השדות הם אופציונליים מכיוון שהקפדנות נשלטת על ידי הגדרות האתר:
...
## סכימות פריטי לקוח (`client-item.ts`)

### צור פריט

סכימה לפריטים שנשלחו על ידי הלקוח עם שדות ליבה נדרשים:
...
מאפשר רק שדות שהלקוחות רשאים לשנות (הכל אופציונלי):
...
מאמת והופך פרמטרי שאילתה עבור רשימות פריטים מעומדים:
...
זהה ל-`createCompanySchema` אך עם שדה `id` וכל שאר השדות אופציונליים:
...
## סכימות מיקום משתמש (`user-location.ts`)
...
מאמת את מיקום פרופיל המשתמש עם חידוד חוצה שדות המבטיח שקו הרוחב והאורך מסופקים יחד:
...
## סכימות מודעות חסות (`sponsor-ad.ts`)
...
## סכימות לוח מחוונים (`client-dashboard.ts`)
...
- `lib/validations/auth.ts` - אימות סיסמה
- `lib/validations/item.ts` - סכימת מיקום פריט
- `lib/validations/client-item.ts` - סכימות פריטים הפונות ללקוח
- `lib/validations/client-dashboard.ts` - סכימות שאילתות של לוח המחוונים
- `lib/validations/company.ts` - סכימות חברה ופריטים-חברה
- `lib/validations/user-location.ts` - סכימות מיקום משתמש
- `lib/validations/sponsor-ad.ts` - סכימות מחזור חיים של מודעות חסות
- `lib/types/item.ts` - `ITEM_VALIDATION` קבועים המשמשים סכמות
