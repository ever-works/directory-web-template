...
title: שאילתות ניהול ולוח מחוונים Deep Dive
sidebar_label: שאילתות אדמין Deep Dive
sidebar_position: 62
---

# שאילתות ניהול ולוח מחוונים Deep Dive

הפניה מקיפה לנתוני לוח המחוונים לניהול, ניהול לקוחות, חיפוש מתקדם, סטטיסטיקות ופונקציות שאילתות דיווח.

## סקירה כללית

שכבת שאילתת המנהל מפעילה את לוח המחוונים של הניהול עם שאילתות אופטימליות בשני מודולים ראשיים:

- **`dashboard.queries.ts`** -- נתונים סטטיסטיים של לוח המחוונים, מדדי מעורבות, תרשימים שבועיים/יומיים ופריטים בעלי ביצועים גבוהים
- **`client.queries.ts`** (קטע ניהול) -- רישום לקוח עם עימוד, נתוני לוח המחוונים של ניהול, חיפוש מתקדם עם 20+ ממדי סינון וסטטיסטיקה מקיפה
...
מקבל סך כל הערות שלא נמחקו שהתקבלו על קבוצה של פריטים.
...
|-------------|------------|----------|----------------------------------|
|`itemSlugs`|`string[]`|כן|מערך של שבלולים של פריט לספור|
...
מקבל את ספירת הפעילות הכוללת של משתמש (הצבעות + הערות של משתמש).
...
|פרמטר|הקלד|חובה|תיאור|
|-------------------|----------|----------|-------------------|
|`clientProfileId`|`string`|כן|מזהה פרופיל לקוח|
...
מקבל נתוני פעילות יומית עבור תרשים פעילות (N הימים האחרונים).
...
|-------------------|------------|----------|---------|--------------------------|
|`clientProfileId`|`string`|כן| --      |מזהה פרופיל לקוח|
|`itemSlugs`|`string[]`|כן| --      |שבלולים של פריט|
|`days`|`number`|לא| `7`     |מספר ימים|

**החזרות:** מערך יומי עם `{ date: 'Mon', submissions: 0, views: 0, engagement: 5 }`

**דפוס SQL:** משתמש ב-`EXTRACT(DOW FROM created_at)` לקיבוץ יום בשבוע ללא תלות באזור.

**הערה:** השדות `submissions` ו-`views` הם מצייני מיקום (תמיד `0`); היישום בפועל נמצא בשכבת המאגר.
...
פונקציית מציין מיקום. פריטים מאוחסנים ב-Git, לא במסד הנתונים.

```typescript
async function getRecentSubmissionsCount(_days: number = 7): Promise<number>
```

**מחזירות:** תמיד `0`. היישום בפועל נמצא בשכבת המאגר.
...
מקבל נתונים סטטיסטיים בסיסיים של פרופיל לקוח.
...
נקודת קצה אופטימלית של לוח המחוונים לניהול המחזירה לקוחות, נתונים סטטיסטיים ועימוד בשיחה אחת. מפחית מסדי נתונים הלוך ושוב.
...
**מסננים נתמכים:**
- חיפוש טקסט (ILKE על פני 5 שדות)
- סטטוס, תוכנית, סוג חשבון
- ספק אישור (קיימת שאילתת משנה)
- טווח תאריכים (נוצר/עודכן אחרי/לפני)
...
1. **דפוס אי הכללת מנהל מערכת** -- כל שאילתות הרישום משתמשות ב-`LEFT JOIN userRoles / roles WHERE roles.id IS NULL` כדי להחריג באופן עקבי משתמשי מנהל מתצוגות לקוח.

2. **שאילתת משנה לספק** -- משתמש בשאילתת המשנה `(SELECT provider FROM accounts WHERE ... LIMIT 1)` במקום JOIN כדי להימנע מכפל שורות מחשבונות OAuth מרובים.

3. **בריחה נכונה של SQL** -- כל כניסות חיפוש הטקסט עוברות אסקייפ עבור תווים מיוחדים של ILIKE (`%`, `_`, `\`) כדי למנוע הזרקת SQL דרך מונחי חיפוש.

4. **טיפול בשבוע ISO** -- `getWeeklyEngagementData` משתמש במסייע מותאם אישית של `getISOWeekString` התואם את `to_char(date, 'IYYY-IW')` של PostgreSQL לטיפול נכון בשנת שבוע ISO בגבולות השנה.

## דוגמאות לשימוש

### טוען את לוח המחוונים לניהול
