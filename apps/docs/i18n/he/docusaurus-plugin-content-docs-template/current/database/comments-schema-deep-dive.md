...
title: "הערות סכימה Deep Dive"
sidebar_label: "סכימת הערות"
sidebar_position: 54
---

# הערות סכימה Deep Dive

## סקירה כללית

מערכת ההערות מאפשרת למשתמשים להשאיר משוב וסקירות על פריטים. הערות מקושרות ל-`client_profiles` (לא ישירות ל-`users`), כוללות שדה דירוג ותומכות במחיקה רכה באמצעות `deletedAt`. תת-מערכת הניהול (`reports` ו-`moderation_history`) מספקת דיווח תוכן ומעקב אחר פעולות מנהליות.
...
מערכת דיווח תוכן הן לפריטים והן להערות.
...
|`contentType`|`content_type`|`text (enum)`|לא| - |`item`, `comment`|
|`contentId`|`content_id`|`text`|לא| - |מזהה התוכן המדווח|
|`reason`|`reason`|`text (enum)`|לא| - |ראה הקובץ למטה|
|`details`|`details`|`text`|כן| - |פרטים שסופקו על ידי המשתמש|
|`status`|`status`|`text (enum)`|לא|`'pending'`|ראה הקובץ למטה|
|`resolution`|`resolution`|`text (enum)`|כן| - |ראה הקובץ למטה|
|`reportedBy`|`reported_by`|`text`|לא| - |FK -> `client_profiles.id` (CASCADE)|
|`reviewedBy`|`reviewed_by`|`text`|כן| - |FK -> `users.id` (SET NULL)|
|`reviewNote`|`review_note`|`text`|כן| - |הערת ביקורת מנהל|
|`createdAt`|`created_at`|`timestamp`|לא|`now()`| - |
|`updatedAt`|`updated_at`|`timestamp`|לא|`now()`| - |
|`reviewedAt`|`reviewed_at`|`timestamp`|כן| - | - |
|`resolvedAt`|`resolved_at`|`timestamp`|כן| - | - |
...
|שם|עמודות|הקלד|
|---|---|---|
|`reports_content_type_idx`|`contentType`|B-עץ|
|`reports_content_id_idx`|`contentId`|B-עץ|
|`reports_status_idx`|`status`|B-עץ|
|`reports_reported_by_idx`|`reportedBy`|B-עץ|
|`reports_created_at_idx`|`createdAt`|B-עץ|
|`reports_content_type_content_id_idx`|`(contentType, contentId)`|עץ B מורכב|
