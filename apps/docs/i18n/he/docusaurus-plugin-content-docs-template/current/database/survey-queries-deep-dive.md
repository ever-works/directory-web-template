...
title: שאילתות סקר Deep Dive
sidebar_label: שאילתות סקר Deep Dive
sidebar_position: 68
---

# שאילתות סקר Deep Dive

התייחסות מקיפה לכל פונקציות שאילתות מסד הנתונים הקשורות לסקר, כולל סקר CRUD, ניהול תגובות, עימוד, סינון וספירת תגובות.

## סקירה כללית

שכבת שאילתות הסקר מנהלת סקרים ותשובותיהם:

- **`survey.queries.ts`** -- סקר CRUD מלא עם מחיקה רכה, רישום מעומד עם מסננים, ספירת תגובות וסטטוס השלמה JOIN, תגובת סקר CRUD עם סינון טווח תאריכים וספירת תגובות

סקרים תומכים בשני סוגים: `global` (ברחבי האתר) ו-`item` (המשויכים לפריט ספציפי). התשובות מקושרות הן לסקר והן לפריט אופציונלי, מה שמאפשר איסוף משוב לכל פריט.
...
מקבל סקר לפי שבלול ה-URL שלו, עם אי הכללה אופציונלית של מזהה סקר ספציפי (שימושי לאימות ייחודיות במהלך עדכונים).
...
|------------|----------|----------|-------------------------------------------|
|`slug`|`string`|כן|שבלול סקר|
|`ignoreId`|`string`|לא|מזהה סקר שלא ייכלל בחיפוש|

**החזרות:** סקר או `null` אם לא נמצא
...
**החזרות:** רשומת הסקר שנוצרה (דרך `RETURNING`)
...
Soft מוחק סקר על ידי הגדרת `deletedAt` לחותמת הזמן הנוכחית.
...
**הערה:** כל שאילתות הקריאה מסננות על `deleted_at IS NULL`, כך שסקרים שנמחקו בצורה רכה מוסתרים למעשה מכל פונקציות האחזור.
...
מקבל תשובות לסקר מעומד עם סינון אופציונלי לפי פריט, משתמש וטווח תאריכים.
...
|-------------|----------|----------|---------|-----------------------------------|
|`surveyId`|`string`|כן| --      |מזהה סקר לקבל תשובות עבורו|
|`itemId`|`string`|לא| --      |סנן לפי פריט|
|`userId`|`string`|לא| --      |סנן לפי משתמש|
|`startDate`|`string`|לא| --      |סנן תגובות לאחר תאריך זה|
|`endDate`|`string`|לא| --      |סנן תגובות לפני תאריך זה|
|`page`|`number`|לא| `1`     |מספר עמוד|
|`limit`|`number`|לא| `10`    |תוצאות לכל עמוד|

**החזרות:** רשימת תגובות עם עימוד
...
**הערות ביצועים:** שאילתות נתונים וספירה פועלות במקביל דרך `Promise.all`.
...
מקבל את המספר הכולל של תגובות לסקר.
