...
Il sistema supporta due provider email configurati tramite `works.yml` e variabili d'ambiente:
...
Il provider viene selezionato in base al campo `mail.provider` in `works.yml`. La configurazione email viene costruita dinamicamente dalla configurazione dell'app usando `createEmailConfig()`.
