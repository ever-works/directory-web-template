...
title: Riferimento alla struttura del progetto
...
Questo documento fornisce un riferimento completo alla struttura delle directory dei modelli Ever Works. Il template è un **Turborepo monorepo** gestito con workspace pnpm. L'applicazione principale è un'app Next.js 16 che utilizza App Router, TypeScript, Drizzle ORM e Tailwind CSS, situata in `apps/web/`.

## Panoramica di primo livello (radice Monorepo)
...
L'applicazione principale si trova in `apps/web/`. Tutte le directory di origine che in precedenza si trovavano nella radice del repository ora risiedono qui.
...
Il modello usa **Turborepo** per l'orchestrazione della compilazione e **spazi di lavoro pnpm** per la gestione delle dipendenze nel monorepo.

|Strumento|Scopo|Configurazione|
|------|---------|---------------|
|Turborepo|Orchestrazione delle attività, memorizzazione nella cache, build parallele|`turbo.json`|
|spazi di lavoro pnpm|Gestione delle dipendenze, collegamento dello spazio di lavoro|`pnpm-workspace.yaml`|
|`packages/tsconfig`|Configurazioni di base TypeScript condivise|Esteso da ciascuna app|
|`packages/eslint-config`|Preimpostazioni di configurazione flat ESLint 9 condivise|Importato da ciascuna app|

### App per l'area di lavoro

- **`apps/web`** -- L'applicazione Next.js 16 principale (elenco delle directory, pannello di amministrazione, percorsi API).
- **`apps/web-e2e`** -- Test end-to-end per drammaturghi destinati a `apps/web`. Separato nel proprio spazio di lavoro per dipendenze isolate ed esecuzioni CI indipendenti.
- **`apps/docs`** -- Sito di documentazione di Docusaurus v3 per il modello stesso.
