...
La classe `ConfigManager` in `lib/config-manager.ts` gestisce il file `works.yml` dal repository CMS basato su Git. Gestisce la lettura, la scrittura e il commit delle modifiche alla configurazione.
