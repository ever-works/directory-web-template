...
Le impostazioni di posizione sono definite nella sezione `settings.location` del file `works.yml` del repository di contenuti:
...
La struttura raw letta da `works.yml`, definita in `lib/types/location.ts`:
