...
title: Approfondimento delle query sull'amministrazione e sulla dashboard
sidebar_label: Query di amministrazione Approfondimento
sidebar_position: 62
---

# Approfondimento delle query sull'amministrazione e sulla dashboard

Riferimento completo per i dati del dashboard di amministrazione, la gestione dei clienti, la ricerca avanzata, le statistiche e le funzioni di query di reporting.

## Panoramica

Il livello di query di amministrazione alimenta il dashboard di amministrazione con query ottimizzate su due moduli principali:

- **`dashboard.queries.ts`** -- Statistiche del dashboard, metriche sul coinvolgimento, grafici settimanali/giornalieri ed elementi con le migliori prestazioni
- **`client.queries.ts`** (sezione amministrazione) -- Elenco clienti con impaginazione, dati del dashboard di amministrazione, ricerca avanzata con oltre 20 dimensioni di filtro e statistiche complete
...
Ottiene il totale dei commenti non eliminati ricevuti su un insieme di elementi.
...
|-------------|------------|----------|----------------------------------|
|`itemSlugs`|`string[]`|Sì|Matrice di lumache di oggetti da contare|
...
Ottiene il conteggio totale delle attività di un utente (voti + commenti fatti dall'utente).
...
|Parametro|Digitare|Obbligatorio|Descrizione|
|-------------------|----------|----------|-------------------|
|`clientProfileId`|`string`|Sì|ID del profilo cliente|
...
Ottiene i dati dell'attività giornaliera per il grafico delle attività (ultimi N giorni).
...
|-------------------|------------|----------|---------|--------------------------|
|`clientProfileId`|`string`|Sì| --      |ID del profilo cliente|
|`itemSlugs`|`string[]`|Sì| --      |Lumache degli oggetti|
|`days`|`number`|No| `7`     |Numero di giorni|

**Resi:** Array giornaliero con `{ date: 'Mon', submissions: 0, views: 0, engagement: 5 }`

**Modello SQL:** Utilizza `EXTRACT(DOW FROM created_at)` per il raggruppamento del giorno della settimana indipendente dalle impostazioni locali.

**Nota:** i campi `submissions` e `views` sono segnaposto (sempre `0`); l'effettiva implementazione è nel livello del repository.
...
Funzione segnaposto. Gli elementi sono archiviati in Git, non nel database.

```typescript
async function getRecentSubmissionsCount(_days: number = 7): Promise<number>
```

**Resi:** Sempre `0`. L'effettiva implementazione è nel livello del repository.
...
Ottiene le statistiche di base del profilo cliente.
...
Endpoint del dashboard di amministrazione ottimizzato che restituisce client, statistiche e impaginazione in un'unica chiamata. Riduce i viaggi di andata e ritorno del database.
...
**Filtri supportati:**
- Ricerca testuale (ILIKE su 5 campi)
- Stato, piano, tipo di account
- Provider di autenticazione (sottoquery EXISTS)
- Intervallo di date (creato/aggiornato dopo/prima)
...
1. **Modello di esclusione amministratore** -- Tutte le query sugli elenchi utilizzano `LEFT JOIN userRoles / roles WHERE roles.id IS NULL` per escludere in modo coerente gli utenti amministratori dalle visualizzazioni dei client.

2. **Sottoquery per il provider**: utilizza la sottoquery `(SELECT provider FROM accounts WHERE ... LIMIT 1)` invece di JOIN per evitare la moltiplicazione delle righe da più account OAuth.

3. **Escape SQL corretto** -- Tutti gli input di ricerca di testo vengono sottoposti a escape per i caratteri speciali ILIKE (`%`, `_`, `\`) per impedire l'inserimento SQL tramite i termini di ricerca.

4. **Gestione della settimana ISO** -- `getWeeklyEngagementData` utilizza un helper personalizzato `getISOWeekString` che corrisponde a `to_char(date, 'IYYY-IW')` di PostgreSQL per la corretta gestione dell'anno della settimana ISO ai confini dell'anno.

## Esempi di utilizzo

### Caricamento della dashboard di amministrazione
