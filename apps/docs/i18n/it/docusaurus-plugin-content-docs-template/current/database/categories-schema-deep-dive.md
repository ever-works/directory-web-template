...
title: "Approfondimento dello schema di categorie e aziende"
sidebar_label: "Schema delle categorie"
sidebar_position: 52
---

# Approfondimento dello schema di categorie e aziende

## Panoramica

Nel modello Ever Works, le **categorie sono definite nel CMS basato su Git** (repository di contenuti), non nel database. Non esiste una tabella del database `categories`. Tuttavia, il database fornisce l’infrastruttura per collegare gli elementi alle aziende e tenere traccia delle gerarchie aziendali, che ha uno scopo organizzativo simile.

Questa pagina documenta la tabella `companies`, la tabella di giunzione `items_companies` e il modo in cui i riferimenti a categoria/azienda vengono visualizzati nello schema.
...
## Tabella: `items_companies`

Tabella di giunzione che collega gli slug degli articoli ai record aziendali. Ogni slug articolo può essere associato a una sola azienda (vincolo unico su `item_slug`).

### Colonne

|Colonna|Nome del DB|Digitare|Nullabile|Predefinito|Vincoli|
|---|---|---|---|---|---|
|`itemSlug`|`item_slug`|`text`|No| - |Unico|
|`companyId`|`company_id`|`text`|No| - |FK -> `companies.id` (CASCATA)|
...
|---|---|---|
|`items_companies_company_id_idx`|`companyId`|B-albero|
...
### Ottieni articoli per un'azienda
...
### Collegare un elemento a un'azienda
