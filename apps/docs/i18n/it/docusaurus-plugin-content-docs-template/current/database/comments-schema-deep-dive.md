...
title: "Approfondimento sullo schema dei commenti"
sidebar_label: "Schema dei commenti"
sidebar_position: 54
---

# Approfondimento sullo schema dei commenti

## Panoramica

Il sistema di commenti consente agli utenti di lasciare feedback e recensioni sugli articoli. I commenti sono collegati a `client_profiles` (non direttamente a `users`), includono un campo di valutazione e supportano l'eliminazione temporanea tramite `deletedAt`. Il sottosistema di moderazione (`reports` e `moderation_history`) fornisce reporting sui contenuti e monitoraggio delle azioni amministrative.
...
## Tabelle di moderazione

### Tabella: `reports`

Sistema di reporting dei contenuti sia per gli articoli che per i commenti.
...
|`contentId`|`content_id`|`text`|No| - |ID del contenuto segnalato|
|`reason`|`reason`|`text (enum)`|No| - |Vedi l'enumerazione di seguito|
|`details`|`details`|`text`|Sì| - |Dettagli forniti dall'utente|
|`status`|`status`|`text (enum)`|No|`'pending'`|Vedi l'enumerazione di seguito|
|`resolution`|`resolution`|`text (enum)`|Sì| - |Vedi l'enumerazione di seguito|
|`reportedBy`|`reported_by`|`text`|No| - |FK -> `client_profiles.id` (CASCATA)|
|`reviewedBy`|`reviewed_by`|`text`|Sì| - |FK -> `users.id` (SET NULL)|
|`reviewNote`|`review_note`|`text`|Sì| - |Nota di revisione dell'amministratore|
|`createdAt`|`created_at`|`timestamp`|No|`now()`| - |
|`updatedAt`|`updated_at`|`timestamp`|No|`now()`| - |
|`reviewedAt`|`reviewed_at`|`timestamp`|Sì| - | - |
|`resolvedAt`|`resolved_at`|`timestamp`|Sì| - | - |

### Segnala enumerazioni
...
|---|---|---|
|`reports_content_type_idx`|`contentType`|B-albero|
|`reports_content_id_idx`|`contentId`|B-albero|
|`reports_status_idx`|`status`|B-albero|
|`reports_reported_by_idx`|`reportedBy`|B-albero|
|`reports_created_at_idx`|`createdAt`|B-albero|
|`reports_content_type_content_id_idx`|`(contentType, contentId)`|B-albero composito|
...
### Ottieni commenti per un articolo
...
### Eliminazione graduale di un commento
...
### Registra un'azione di moderazione
