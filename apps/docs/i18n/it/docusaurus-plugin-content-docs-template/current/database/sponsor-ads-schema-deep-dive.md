...
title: "Approfondimento sullo schema degli annunci degli sponsor"
sidebar_label: "Schema degli annunci sponsor"
sidebar_position: 58
---

# Approfondimento sullo schema degli annunci degli sponsor

## Panoramica

Il modulo degli annunci sponsor consente agli utenti di pagare per il posizionamento promosso di articoli sul sito. Implementa un ciclo di vita completo: invio, pagamento, revisione amministrativa, visualizzazione attiva e scadenza/cancellazione. Il sistema si integra con l'infrastruttura del fornitore di servizi di pagamento e supporta intervalli di sponsorizzazione sia settimanali che mensili.
...
### Due chiavi esterne per `users`

La tabella contiene due riferimenti alla tabella `users`:
1. **`userId` (CASCADE):** L'utente che ha inviato e paga la sponsorizzazione. L'eliminazione dell'utente comporta l'eliminazione di tutti i suoi annunci sponsor.
2. **`reviewedBy` (SET NULL):** L'amministratore che ha approvato/rifiutato. Se l'amministratore viene eliminato, il record di revisione viene conservato con un revisore nullo.

### Integrazione dei pagamenti esterni

La combinazione `paymentProvider` + `subscriptionId` ha un indice univoco, che rispecchia il modello nella tabella `subscriptions`. Ciò consente di cercare gli annunci degli sponsor in base al riferimento dell'abbonamento esterno durante l'elaborazione del webhook.

### Importo in dollari

A differenza della tabella `subscriptions` che memorizza gli importi in centesimi, la colonna `sponsor_ads.amount` memorizza i valori in dollari interi. Tieni presente questa differenza quando ti integri con i fornitori di pagamenti.
...
- **Identificazione dell'elemento tramite slug.** Come tutte le tabelle che fanno riferimento agli elementi, `itemSlug` memorizza lo slug del contenuto dell'elemento, non una chiave esterna del database.
- **Provider unico+indice di abbonamento.** Garantisce l'assenza di record di abbonamento duplicati e consente ricerche veloci di webhook.
- **Transizioni di stato soft.** I cambiamenti di stato vengono monitorati tramite la colonna di stato stessa. Per un audit trail completo, prendi in considerazione l'abbinamento con la tabella `activityLogs`.
- **Scadenza manuale.** Non esiste un meccanismo di scadenza automatica a livello di database. Un processo cron o un gestore webhook dovrebbe verificare periodicamente la presenza e la scadenza degli annunci sponsor scaduti.
