---
id: crm-types
title: Definizioni del tipo di integrazione CRM
sidebar_label: Tipi di CRM
sidebar_position: 9
---

# Definizioni del tipo di integrazione CRM

**File sorgente:**
- `lib/types/twenty-crm-config.types.ts` - Configurazione e tipi di connessione
- `lib/types/twenty-crm-entities.types.ts` - Tipi di entità Persona e Società
- `lib/types/twenty-crm-errors.types.ts` - Codici di errore, errori strutturati e protezioni del tipo
- `lib/types/twenty-crm-sync.types.ts` - Operazioni di upsert, voci della cache e opzioni di sincronizzazione

Il modello si integra con [Twenty CRM](https://twenty.com/), una piattaforma CRM open source. Questi tipi definiscono l'interfaccia completa per le operazioni di configurazione, mappatura delle entità, gestione degli errori e sincronizzazione.
...
## Tipi di entità (`twenty-crm-entities.types.ts`)

### `TwentyPerson`

Rappresenta un contatto/persona in Twenty CRM:
...
Rappresenta un'azienda/organizzazione in Twenty CRM:
...
## Tipi di sincronizzazione (`twenty-crm-sync.types.ts`)

### `UpsertResult<T>`

Risultato generico di un'operazione upsert (crea o aggiorna):
...
Input per l'aggiornamento di un record aziendale:
...
Ingresso per l'inserimento di un record di persona/contatto:
...
Voce della cache di base per la mappatura degli ID esterni sugli ID CRM:
...
Voce nella cache per le entità Persona:
...
Voce cache per entità aziendali:
...
Unione di tutti i tipi di voci della cache:
...
Opzioni per controllare il comportamento di inserimento:
...
### Utilizzando il tipo di risposta generico
...
### Mappatura ID esterni

L'integrazione CRM utilizza i campi `external_id` per mantenere la mappatura bidirezionale tra entità locali e record CRM. Ciò consente:

- **Upsert idempotenti**: lo stesso ID esterno è sempre associato allo stesso record CRM
- **Ottimizzazione della cache**: le mappature degli ID esterni a CRM vengono memorizzate nella cache per ridurre le chiamate API
- **Risoluzione dei conflitti** - 409 conflitti durante gli aggiornamenti paralleli vengono ritentati automaticamente

### Riprova la strategia

Il client utilizza il backoff esponenziale con le seguenti impostazioni predefinite:
- Backoff iniziale: 200 ms
- Backoff massimo: 5000 ms
- Numero massimo di tentativi: 3
- Il flag `isRetryable` sugli errori indica se il nuovo tentativo automatico è appropriato

### Classificazione degli errori

Gli errori sono classificati da `TwentyCrmErrorCode` per consentire una gestione adeguata:
- `AUTH_ERROR` - Chiave API non valida o scaduta
- `RATE_LIMIT` - Troppe richieste (riprovabile)
- `TIMEOUT` - Richiesta scaduta (riprovabile)
- `VALIDATION_ERROR` - Dati non validi inviati al CRM
- `NOT_FOUND` - L'entità non esiste
- `NETWORK_ERROR` - Problemi di connessione (riprovabile)

## Tipi correlati

- [`ClientProfileWithAuth`](./user-types.md) - Profilo client locale mappato a `TwentyPerson`
- [`CreateClientRequest`](./user-types.md) - Campi di creazione del client che alimentano la sincronizzazione CRM
