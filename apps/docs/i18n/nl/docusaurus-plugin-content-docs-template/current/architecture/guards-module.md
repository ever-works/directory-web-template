---
id: guards-module
title: Bewakingsmodule
sidebar_label: Bewakingsmodule
sidebar_position: 58
---

# Bewakingsmodule

De bewakersmodule (`template/lib/guards/`) biedt een plangebaseerd toegangscontrolesysteem. Het definieert welke abonnementen toegang hebben tot welke functies, handhaaft numerieke limieten per abonnement en biedt zowel bewakingsfuncties aan de serverzijde als een React-vriendelijke resultatenbouwer voor gebruik aan de clientzijde.
...
|------|-------------|
|`lib/guards/index.ts`|Export van vaten|
|`lib/guards/plan-features.guard.ts`|Volledige bewakingsimplementatie|

## Planhiërarchie

Plannen zijn geordend op niveau, waarbij hogere niveaus alle functies van lagere niveaus omvatten, indien geconfigureerd met `minPlan`:
...
Alle beschikbare functies worden als constanten gedeclareerd:
...
De `FEATURE_ACCESS`-record wijst elke functie toe aan de bijbehorende toegangsregel:
...
### Samenvatting van toegangsregels

|Functie|Toegangsregel|
|---------|------------|
|`submit_product`|`'all'`|
...
Numerieke limieten voor functies met hoeveelheidsbeperkingen. `null` geeft onbeperkt aan.
...
Kerntoegangscontrole die de functietoegangsregel evalueert:
...
Retourneert de limietwaarde voor een plan:
...
Controleert of een waarde binnen de limiet van het plan valt:
...
Retourneert alle functies die toegankelijk zijn via een bepaald abonnement.

### `getMinimumPlanForFeature(feature: Feature): PaymentPlan`

Retourneert het laagste abonnement dat toegang heeft tot een functie.

## Plan Guardfabriek

### `createPlanGuard(userPlan: string)`

Creëert een guard-instantie met gebonden methoden voor een specifiek plan:
...
Aangepaste fout gegenereerd door `requireFeature`:
...
Creëert een gewoon object dat geschikt is voor gebruik in React-hooks en -componenten:
