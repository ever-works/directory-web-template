...
title: Referentie projectstructuur
sidebar_label: Projectstructuur
sidebar_position: 4
---

# Referentie projectstructuur

Dit document biedt een volledige referentie van de Ever Works-sjabloonmapstructuur. De sjabloon is een **Turborepo monorepo** beheerd met pnpm-werkruimten. De primaire applicatie is een Next.js 16-app die gebruikmaakt van de App Router, TypeScript, Drizzle ORM en Tailwind CSS, gelegen in `apps/web/`.

## Overzicht op het hoogste niveau (Monorepo Root)
...
De primaire applicatie bevindt zich in `apps/web/`. Alle bronmappen die zich voorheen in de hoofdmap van de repository bevonden, bevinden zich nu hier.
...
De sjabloon gebruikt **Turborepo** voor build-orkestratie en **pnpm-werkruimten** voor afhankelijkheidsbeheer in de monorepo.

|Gereedschap|Doel|Configuratie|
|------|---------|---------------|
|Turborepo|Taakorkestratie, caching, parallelle builds|`turbo.json`|
|pnpm-werkruimten|Afhankelijkheidsbeheer, koppeling van werkruimten|`pnpm-workspace.yaml`|
|`packages/tsconfig`|Gedeelde TypeScript-basisconfiguraties|Uitgebreid per app|
|`packages/eslint-config`|Gedeelde ESLint 9 platte configuratievoorinstellingen|Geïmporteerd door elke app|

### Werkruimte-apps

- **`apps/web`** -- De belangrijkste Next.js 16-applicatie (directorylijst, beheerderspaneel, API-routes).
- **`apps/web-e2e`** -- End-to-end tests van toneelschrijvers gericht op `apps/web`. Gescheiden in een eigen werkruimte voor geïsoleerde afhankelijkheden en onafhankelijke CI-uitvoeringen.
- **`apps/docs`** -- Docusaurus v3-documentatiesite voor de sjabloon zelf.
