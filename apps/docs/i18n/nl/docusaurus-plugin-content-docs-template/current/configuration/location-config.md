...
Locatie-instellingen worden gedefinieerd in de sectie `settings.location` van het `works.yml`-bestand van uw contentrepository:
...
De ruwe structuur gelezen uit `works.yml`, gedefinieerd in `lib/types/location.ts`:
