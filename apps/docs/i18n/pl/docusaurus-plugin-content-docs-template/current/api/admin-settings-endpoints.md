...
Punkty końcowe ustawień administratora zarządzają konfiguracją aplikacji przechowywaną w `works.yml`, w tym ogólnymi ustawieniami witryny oraz statusem funkcji mapy.
...
Zwraca pełną konfigurację aplikacji z `works.yml`.
...
Aktualizuje pojedynczy klucz konfiguracji. Klucz jest automatycznie poprzedzany prefiksem `"settings."` przed zapisem w `works.yml`.
...
| Warstwa | Plik | Opis |
|---------|------|------|
| Konfiguracja aplikacji | `works.yml` | Ustawienia runetime zarządzane przez panel admina |
...
Ustawienia przechowywane w `works.yml` są odczytywane przez serwer przy każdym żądaniu (bez buforowania w panelu admina), zapewniając natychmiastowe działanie zmian bez konieczności ponownego wdrożenia.
...
| 500 | Nie można odczytać lub zapisać `works.yml` |
