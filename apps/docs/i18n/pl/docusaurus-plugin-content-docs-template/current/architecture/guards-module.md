---
id: guards-module
title: Moduł Strażników
sidebar_label: Moduł Strażników
sidebar_position: 58
---

# Moduł Strażników

Moduł strażników (`template/lib/guards/`) zapewnia oparty na planie system kontroli dostępu. Definiuje, które plany subskrypcji mogą uzyskać dostęp do jakich funkcji, egzekwuje limity liczbowe na plan i zapewnia zarówno funkcje ochronne po stronie serwera, jak i przyjazne dla React narzędzie do tworzenia wyników do użytku po stronie klienta.
...
|Plik|Opis|
|------|-------------|
|`lib/guards/index.ts`|Eksport beczek|
|`lib/guards/plan-features.guard.ts`|Kompletne wdrożenie strażnika|

## Hierarchia planu

Plany są uporządkowane według poziomów, gdzie wyższe poziomy obejmują wszystkie funkcje niższych poziomów, jeśli są skonfigurowane z `minPlan`:
...
Wszystkie dostępne funkcje są deklarowane jako stałe:
...
Rekord `FEATURE_ACCESS` odwzorowuje każdą funkcję na jej regułę dostępu:
...
Limity liczbowe dla obiektów z ograniczeniami ilościowymi. `null` oznacza nieograniczone.
...
## Dostęp do funkcji sprawdzania

### `canAccessFeature(feature: Feature, userPlan: string): boolean`

Kontrola dostępu do rdzenia, która ocenia regułę dostępu do funkcji:
...
Zwraca wartość limitu dla planu:
...
Sprawdza, czy wartość mieści się w limicie planu:
...
Zwraca wszystkie funkcje dostępne w ramach danego planu.

### `getMinimumPlanForFeature(feature: Feature): PaymentPlan`

Zwraca najniższy plan, który może uzyskać dostęp do funkcji.

## Planuj fabrykę strażników

### `createPlanGuard(userPlan: string)`

Tworzy instancję strażnika z powiązanymi metodami dla określonego planu:
...
Niestandardowy błąd zgłoszony przez `requireFeature`:
...
Tworzy zwykły obiekt odpowiedni do użycia w hakach i komponentach React:
