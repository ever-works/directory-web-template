...
title: Odniesienie do struktury projektu
sidebar_label: Struktura projektu
sidebar_position: 4
---

# Odniesienie do struktury projektu

Ten dokument zawiera pełne informacje na temat struktury katalogów szablonów Ever Works. Szablon to **monorepo Turborepo** zarządzane za pomocą obszarów roboczych pnpm. Podstawową aplikacją jest aplikacja Next.js 16 korzystająca z App Router, TypeScript, Drizzle ORM i Tailwind CSS, zlokalizowana w `apps/web/`.

## Przegląd najwyższego poziomu (root Monorepo)
...
Główna aplikacja znajduje się w `apps/web/`. Wszystkie katalogi źródłowe, które wcześniej znajdowały się w katalogu głównym repozytorium, teraz znajdują się tutaj.
...
Szablon wykorzystuje **Turborepo** do orkiestracji kompilacji i **obszary robocze pnpm** do zarządzania zależnościami w monorepo.

|Narzędzie|Cel|Konfiguracja|
|------|---------|---------------|
|Turborepo|Orkiestracja zadań, buforowanie, kompilacje równoległe|`turbo.json`|
|obszary robocze pnpm|Zarządzanie zależnościami, łączenie obszarów roboczych|`pnpm-workspace.yaml`|
|`packages/tsconfig`|Udostępnione konfiguracje podstawowe TypeScript|Rozszerzone o każdą aplikację|
|`packages/eslint-config`|Udostępnione płaskie ustawienia konfiguracji ESLint 9|Importowane przez każdą aplikację|

### Aplikacje obszaru roboczego

- **`apps/web`** -- Główna aplikacja Next.js 16 (lista katalogów, panel administracyjny, trasy API).
- **`apps/web-e2e`** — Kompleksowe testy dramaturga skierowane do `apps/web`. Podzielony na własny obszar roboczy dla izolowanych zależności i niezależnych przebiegów CI.
- **`apps/docs`** -- Witryna z dokumentacją Docusaurus v3 dotycząca samego szablonu.
