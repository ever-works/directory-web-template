...
Szablon Ever Works przetwarza przychodzące webhooki od trzech dostawców płatności: **Stripe**, **Lemon Squeezy** i **Polar**. Każdy dostawca ma dedykowaną trasę API, która weryfikuje podpisy, normalizuje typy zdarzeń do wspólnego wyliczenia `WebhookEventType` i wysyła do funkcji obsługi w celu zarządzania subskrypcjami, śledzenia płatności i powiadomień e-mail.
...
|Plik|Cel|
|------|---------|
|`template/app/api/stripe/webhook/route.ts`|Obsługa webhooka w paski|
|`template/app/api/lemonsqueezy/webhook/route.ts`|Obsługa webhooka LemonSqueezy|
|`template/app/api/polar/webhook/route.ts`|Punkt wejścia webhooka Polar|
|`template/app/api/polar/webhook/router.ts`|Kierowanie wydarzeniami polarnymi|
|`template/app/api/polar/webhook/handlers.ts`|Obsługa zdarzeń polarnych|
|`template/app/api/polar/webhook/types.ts`|Definicje typów webhooków Polar|
|`template/app/api/polar/webhook/utils.ts`|Funkcje użytkowe bieguna|

## Typowe typy zdarzeń

Wszyscy dostawcy normalizują swoje zdarzenia do wspólnego wyliczenia `WebhookEventType`:

|Typ zdarzenia webhooka|Pasek|Wyciskacz cytrynowy|Polarny|
|------------------|--------|--------------|-------|
...
## Przetwarzanie webhooka w paski
...
Każdy moduł obsługi ma spójny wzór:

1. Sprawdź, czy jest to subskrypcja reklam sponsora (specjalna obsługa)
2. Aktualizuj zapisy subskrypcji poprzez `WebhookSubscriptionService`
3. Wyodrębnij informacje o klientach i przygotuj dane e-mailowe
4. Wyślij odpowiedni e-mail z powiadomieniem
5. Zapisz sukces lub porażkę
...
## Przetwarzanie webhooka LemonSqueezy

### Mapowanie typu zdarzenia

LemonSqueezy używa różnych nazw zdarzeń, które są mapowane na wspólne wyliczenie:
...
### Niestandardowy dostęp do danych

LemonSqueezy używa `custom_data` i `meta.custom_data` do metadanych (zamiast `metadata` Stripe):
...
## Przetwarzanie webhooka Polar

Polar wykorzystuje bardziej uporządkowaną architekturę z oddzielnymi plikami do routingu, obsługi i typów.
...
Przed wysłaniem router sprawdza typy zdarzeń na podstawie listy dozwolonych, zapobiegając niezweryfikowanym wywołaniom metod dynamicznych.

### Weryfikacja podpisu (Polar)
...
### Odporna obsługa poczty e-mail

Programy obsługi Polar otaczają operacje e-mail w zagnieżdżonych blokach try/catch, dzięki czemu awarie e-maili nigdy nie zawiodą webhooka:
...
## Obsługa reklam sponsorskich

Wszyscy trzej dostawcy wykrywają subskrypcje reklam sponsorów za pomocą metadanych i kierują je do dedykowanych modułów obsługi:

|Akcja|Funkcja|Opis|
|--------|----------|-------------|
|Płatność potwierdzona|`handleSponsorAdActivation()`|Ustawia stan reklamy na oczekującą na sprawdzenie|
|Subskrypcja anulowana|`handleSponsorAdCancellation()`|Anuluje reklamę sponsora|
|Płatność odnowiona|`handleSponsorAdRenewal()`|Przedłuża datę końcową reklamy|

## Najlepsze praktyki

1. **Zawsze weryfikuj podpisy** – nigdy nie przetwarzaj niezweryfikowanych webhooków
2. **Użyj surowej treści do weryfikacji podpisu** – po weryfikacji przeanalizuj JSON osobno
3. **Zwróć szybko 200** – dostawcy usług płatniczych ponawiają próbę w przypadku odpowiedzi innych niż 2xx
4. **Izoluj błędy w e-mailach** – zawijaj wysyłanie e-maili w zagnieżdżonym trybie try/catch
5. **Sprawdź typy zdarzeń** – przed wysłaniem sprawdź listę dozwolonych
6. **Dziennik z danymi strukturalnymi** — uwzględniaj identyfikatory i typy zdarzeń we wszystkich wpisach dziennika
7. **Użyj dostawców singletonu** -- `getOrCreateStripeProvider()` zapobiega wielokrotnym instancjom
