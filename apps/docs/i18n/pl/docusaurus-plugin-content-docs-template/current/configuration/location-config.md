...
Ustawienia lokalizacji są zdefiniowane w sekcji `settings.location` pliku `works.yml` w repozytorium treści:
...
Surowa struktura odczytywana z `works.yml`, zdefiniowana w `lib/types/location.ts`:
