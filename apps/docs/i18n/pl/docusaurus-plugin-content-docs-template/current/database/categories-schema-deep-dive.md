...
title: "Kategorie i firmy Schemat Głębokie nurkowanie"
sidebar_label: "Schemat kategorii"
sidebar_position: 52
---

# Kategorie i firmy Schemat Głębokie nurkowanie

## Przegląd

W szablonie Ever Works **kategorie są zdefiniowane w CMS-ie opartym na Git** (repozytorium treści), a nie w bazie danych. Brak tabeli bazy danych `categories`. Jednakże baza danych zapewnia infrastrukturę umożliwiającą łączenie elementów z firmami i śledzenie hierarchii firm, co służy podobnemu celowi organizacyjnemu.

Ta strona dokumentuje tabelę `companies`, tabelę skrzyżowań `items_companies` oraz sposób, w jaki odniesienia do kategorii/firmy pojawiają się w całym schemacie.
...
Tabela połączeń łącząca ślimaki przedmiotów z dokumentacją firmy. Każdy ślimak pozycji może być powiązany tylko z jedną firmą (unikalne ograniczenie `item_slug`).

### Kolumny

|Kolumna|Nazwa bazy danych|Wpisz|Możliwość wartości null|Domyślne|Ograniczenia|
|---|---|---|---|---|---|
|`itemSlug`|`item_slug`|`text`|Nie| - |Wyjątkowy|
|`companyId`|`company_id`|`text`|Nie| - |FK -> `companies.id` (KASKADA)|
...
|Imię|Kolumny|Wpisz|
|---|---|---|
|`items_companies_company_id_idx`|`companyId`|Drzewo B|
...
### Pobierz wszystkie aktywne firmy
