...
title: "Głębokie zanurzenie się w schemacie głosów"
sidebar_label: "Schemat głosów"
sidebar_position: 55
---

# Głębokie zanurzenie się w schemacie głosów

## Przegląd

System głosowania implementuje mechanizm głosowania za/przeciw dla elementów. Każdy użytkownik (identyfikowany poprzez swój rekord `client_profiles`) może oddać dokładnie jeden głos na pozycję, co jest egzekwowane przez unikalny indeks złożony. Głosy można przełączać pomiędzy głosami za i przeciw.
...
- **Głosy odnoszą się do profili klientów, a nie użytkowników.** Jest to zgodne z tabelą komentarzy. Aby móc głosować, użytkownicy muszą posiadać rekord `client_profiles`.
- **Elementy są identyfikowane przez ślimak.** Kolumna `item_id` przechowuje informację o produkcie z Git CMS, a nie klucz obcy bazy danych.
- **Brak osobnej tabeli zliczania głosów.** Zliczanie głosów jest agregowane w czasie wykonywania zapytania za pomocą `count()`. To zamienia koszt zapytania na spójność (bez przestarzałych liczników).
- **Pole `updatedAt` śledzi zmiany głosów.** Kiedy użytkownik przełącza się z głosowania za na głosowanie przeciw, `updatedAt` jest aktualizowane, a `createdAt` zachowuje pierwotny czas głosowania.
