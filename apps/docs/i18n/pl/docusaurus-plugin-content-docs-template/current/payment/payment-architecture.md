...
Szablon Ever Works implementuje system płatności niezależny od dostawcy, który obsługuje czterech dostawców płatności: **Stripe**, **LemonSqueezy**, **Polar** i **Solidgate**. Architektura wykorzystuje wzorzec fabryczny z pojedynczymi instancjami dostawców, umożliwiając przełączanie dostawców w czasie wykonywania.
...
Każdy dostawca usług płatniczych wdraża te same `PaymentProviderInterface` :
...
Dzięki temu interfejsowi zmiana dostawcy nie wymaga żadnych zmian w kodzie wywołującym.

##Fabryka dostawcy płatności

Fabryka tworzy instancje dostawcy w oparciu o ciąg znaków typu dostawcy:
...
## Usługa płatności (fasada) `PaymentService` otacza instancję pojedynczego dostawcy za czystym interfejsem API. Deleguje każde wywołanie do podstawowego dostawcy:
...
Usługa definiuje także stałe planu płatności:
...
## Menedżer usług płatności

Menedżer obsługuje cykl życia singletona i przełączanie dostawców środowiska wykonawczego. Utrzymuje wybranego dostawcę w `localStorage` :
...
Menedżer obsługuje renderowanie po stronie serwera, domyślnie korzystając ze skonfigurowanego domyślnego dostawcy, gdy `localStorage` jest niedostępne:
...
## PaymentProviderManager (warstwa konfiguracji)

Wartość `PaymentProviderManager` zapewnia alternatywny wzorzec dostępu singleton z leniwą inicjalizacją i weryfikacją konfiguracji dla każdego dostawcy:
...
Moduł konfiguracyjny eksportuje funkcje pomocnicze w celu szybkiego dostępu do dostawców:
...
Wewnętrzna klasa `ConfigManager` ładuje i sprawdza zmienne środowiskowe dla wszystkich dostawców. Konfiguracja każdego dostawcy jest sprawdzana przy pierwszym dostępie, a nie przy uruchomieniu:
...
Walidacja jest uruchamiana raz dla każdego dostawcy i powoduje błędy opisowe zawierające listę brakujących zmiennych.
...
Każdy dostawca zwraca zestaw komponentów interfejsu użytkownika do renderowania formularzy płatności:
...
Aby dodać piątego dostawcę płatności, wykonaj następujące kroki:

### 1. Utwórz klasę dostawcy
...
### 5. Utwórz komponent interfejsu użytkownika
...
Moduł typów płatności zawiera pomoce formatowania:
