...
Estes endpoints de admin gerenciam links de navegação personalizados do site e o índice de localização geográfica. Os endpoints de navegação permitem configurar links personalizados de cabeçalho e rodapé armazenados em `works.yml`. Os endpoints do índice de localização gerenciam o índice espacial usado para análise geográfica e recursos de mapa.
...
Recupera os itens de navegação `custom_header` e `custom_footer` do arquivo `works.yml` do site. Retorna arrays vazios se nenhuma navegação personalizada estiver configurada.
...
Atualiza os itens de navegação personalizada do cabeçalho ou rodapé em `works.yml`. Valida o formato do caminho de cada item para evitar ataques XSS por meio de esquemas de URL perigosos.
...
- **Armazenamento de Configuração:** Os itens de navegação são armazenados em `works.yml` sob as chaves `custom_header` e `custom_footer`, persistidos via `configManager.updateNestedKey()`.
