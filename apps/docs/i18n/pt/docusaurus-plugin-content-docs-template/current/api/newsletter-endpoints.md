...
O sistema suporta dois provedores de e-mail configurados via `works.yml` e variáveis de ambiente:
...
O provedor é selecionado com base no campo `mail.provider` em `works.yml`. A configuração de e-mail é construída dinamicamente a partir da configuração do app usando `createEmailConfig()`.
