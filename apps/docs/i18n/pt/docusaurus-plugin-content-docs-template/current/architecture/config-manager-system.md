...
O Config Manager System fornece duas camadas de configuração complementares: a classe **ConfigManager** (`lib/config-manager.ts`) para gerenciar o arquivo de configuração de conteúdo baseado em YAML (`works.yml`) com persistência apoiada por Git, e o **ConfigService** (`lib/config/`) para validar e acessar a configuração de aplicativo baseada em variável de ambiente com esquemas Zod. Juntos, eles abrangem configurações editáveis ​​em tempo de execução e configuração do ambiente em tempo de implantação.
