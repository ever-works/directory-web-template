---
id: guards-module
title: Módulo de Guardas
sidebar_label: Módulo de Guardas
sidebar_position: 58
---

# Módulo de Guardas

O módulo de guardas (`template/lib/guards/`) fornece um sistema de controle de acesso de recursos baseado em plano. Ele define quais planos de assinatura podem acessar quais recursos, impõe limites numéricos por plano e fornece funções de proteção do lado do servidor e um construtor de resultados amigável ao React para uso do lado do cliente.
...
|Arquivo|Descrição|
|------|-------------|
|`lib/guards/index.ts`|Exportações de barril|
|`lib/guards/plan-features.guard.ts`|Implementação completa de guarda|

## Hierarquia do plano

Os planos são ordenados por nível, onde os níveis superiores incluem todos os recursos dos níveis inferiores quando configurados com `minPlan`:
...
Todos os recursos disponíveis são declarados como constantes:
...
O registro `FEATURE_ACCESS` mapeia cada recurso para sua regra de acesso:
...
Limites numéricos para recursos com restrições de quantidade. `null` indica ilimitado.
...
Verificação de acesso principal que avalia a regra de acesso ao recurso:
...
Retorna o valor limite de um plano:
...
Retorna todos os recursos acessíveis por um determinado plano.

### `getMinimumPlanForFeature(feature: Feature): PaymentPlan`

Retorna o plano mais baixo que pode acessar um recurso.

## Planejar Fábrica de Guardas

### `createPlanGuard(userPlan: string)`

Cria uma instância de guarda com métodos vinculados para um plano específico:
...
Erro personalizado gerado por `requireFeature`:
...
Cria um objeto simples adequado para uso em ganchos e componentes do React:
