...
title: Definições de permissão
sidebar_label: Definições de permissão
sidebar_position: 32
---

# Definições de permissão

O modelo inclui um sistema de permissão refinado organizado em definições baseadas em recursos, grupos amigáveis à interface do usuário e utilitários de gerenciamento de estado. Este sistema controla o acesso a recursos administrativos, gerenciamento de conteúdo e análises.
...
## Definições de permissão (`definitions.ts`)

Todas as permissões seguem uma convenção de nomenclatura `resource:action`. O objeto `PERMISSIONS` é a única fonte da verdade:
...
O tipo `Permission` é derivado automaticamente do objeto `PERMISSIONS`, garantindo segurança de tipo sem duplicação manual:
...
Duas definições de função integradas fornecem padrões sensatos:
...
## Grupos de permissão (`groups.ts`)

Os grupos de permissões organizam permissões para exibição em componentes da UI administrativa, como editores de função:
...
## Utilitários de estado de permissão (`utils.ts`)

Essas funções gerenciam o estado de alternância de permissão em UIs administrativas (como criação de função ou edição de formulários):

### Criando estado de permissão

Converta uma matriz de permissões em um mapa booleano para UIs baseadas em caixas de seleção:
...
Determine o que foi adicionado e removido ao comparar conjuntos de permissões:
...
Verifique se duas matrizes de permissão contêm as mesmas permissões, independentemente da ordem:
...
Pesquise permissões por palavra-chave:
...
### Verificando permissões em um componente
...
### Construindo um Editor de Função
...
- `lib/permissions/definitions.ts` - Constantes de permissão, tipos e funções padrão
- `lib/permissions/groups.ts` - Grupos de UI e auxiliares de formatação
- `lib/permissions/utils.ts` - Utilitários de gerenciamento de estado
- `lib/guards/` - Protetores de rotas e componentes que consomem permissões
