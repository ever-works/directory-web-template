...
title: Referência da Estrutura do Projeto
...
Este documento fornece uma referência completa da estrutura de diretórios do modelo Ever Works. O modelo é um **Turborepo monorepo** gerenciado com espaços de trabalho pnpm. O aplicativo principal é um aplicativo Next.js 16 usando App Router, TypeScript, Drizzle ORM e Tailwind CSS, localizado em `apps/web/`.

## Visão geral de nível superior (raiz Monorepo)
...
### apps/web/ – Aplicativo Next.js

O aplicativo principal reside em `apps/web/`. Todos os diretórios de origem que estavam anteriormente na raiz do repositório agora residem aqui.
...
O modelo usa **Turborepo** para orquestração de build e **espaços de trabalho pnpm** para gerenciamento de dependências no monorepo.

|Ferramenta|Objetivo|Configuração|
|------|---------|---------------|
|Turborepo|Orquestração de tarefas, cache, compilações paralelas|`turbo.json`|
|espaços de trabalho pnpm|Gerenciamento de dependências, vinculação de espaço de trabalho|`pnpm-workspace.yaml`|
|`packages/tsconfig`|Configurações básicas compartilhadas do TypeScript|Estendido por cada aplicativo|
|`packages/eslint-config`|Predefinições de configuração plana compartilhadas do ESLint 9|Importado por cada aplicativo|

### Aplicativos de espaço de trabalho

- **`apps/web`** - O aplicativo principal Next.js 16 (listagem de diretórios, painel de administração, rotas de API).
- **`apps/web-e2e`** -- Testes completos de dramaturgo direcionados a `apps/web`. Separado em seu próprio espaço de trabalho para dependências isoladas e execuções de CI independentes.
- **`apps/docs`** -- Site de documentação do Docusaurus v3 para o próprio modelo.
