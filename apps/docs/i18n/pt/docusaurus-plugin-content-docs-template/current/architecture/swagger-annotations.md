...
title: Anotações de arrogância
sidebar_label: Anotações de arrogância
sidebar_position: 36
---

# Anotações de arrogância

O modelo inclui um módulo utilitário para gerar anotações OpenAPI/Swagger JSDoc padronizadas para rotas de API do Next.js App Router. Este sistema evita a duplicação de esquemas e parâmetros de resposta comuns.
...
Gera uma string de anotação JSDoc Swagger completa a partir de um objeto de configuração:
...
Uma abreviação para rotas somente de administrador que adiciona automaticamente segurança `sessionAuth`:
...
O objeto `CommonAnnotations` fornece modelos e parâmetros de resposta reutilizáveis:
...
Cada resposta inclui um corpo de exemplo:
...
As anotações são colocadas como comentários JSDoc acima das exportações do manipulador de rota. O gerador Swagger os processa durante a construção para produzir a especificação OpenAPI:
...
## Construindo anotações personalizadas

Combine modelos comuns com configuração específica de rota:
...
- `lib/swagger/annotations.ts` - Tipos de anotação, geradores e modelos comuns
- `app/api/` - Manipuladores de rotas de API que usam as anotações
