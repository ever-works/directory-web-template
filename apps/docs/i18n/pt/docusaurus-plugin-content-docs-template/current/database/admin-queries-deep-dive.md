...
title: Aprofundamento nas consultas de administração e painel
sidebar_label: Aprofundamento das consultas do administrador
sidebar_position: 62
---

# Aprofundamento nas consultas de administração e painel

Referência abrangente para dados do painel de administração, gerenciamento de clientes, pesquisa avançada, estatísticas e funções de consulta de relatórios.

## Visão geral

A camada de consulta administrativa alimenta o painel de administração com consultas otimizadas em dois módulos principais:

- **`dashboard.queries.ts`** -- Estatísticas do painel, métricas de engajamento, gráficos semanais/diários e itens de melhor desempenho
- **`client.queries.ts`** (seção administrativa) – Listagem de clientes com paginação, dados do painel de administração, pesquisa avançada com mais de 20 dimensões de filtro e estatísticas abrangentes
...
Obtém o total de comentários não excluídos recebidos em um conjunto de itens.
...
|-------------|------------|----------|----------------------------------|
|`itemSlugs`|`string[]`|Sim|Matriz de slugs de itens para contar|
...
Obtém a contagem total de atividades de um usuário (votos + comentários feitos pelo usuário).
...
|Parâmetro|Tipo|Obrigatório|Descrição|
|-------------------|----------|----------|-------------------|
|`clientProfileId`|`string`|Sim|ID do perfil do cliente|
...
Obtém dados de atividades diárias para gráfico de atividades (últimos N dias).
...
|-------------------|------------|----------|---------|--------------------------|
|`clientProfileId`|`string`|Sim| --      |ID do perfil do cliente|
|`itemSlugs`|`string[]`|Sim| --      |Lesmas de itens|
|`days`|`number`|Não| `7`     |Número de dias|

**Retornos:** Matriz diária com `{ date: 'Mon', submissions: 0, views: 0, engagement: 5 }`

**Padrão SQL:** Usa `EXTRACT(DOW FROM created_at)` para agrupamento de dias da semana independente de localidade.

**Observação:** os campos `submissions` e `views` são espaços reservados (sempre `0`); a implementação real está na camada do repositório.
...
Função de espaço reservado. Os itens são armazenados no Git, não no banco de dados.

```typescript
async function getRecentSubmissionsCount(_days: number = 7): Promise<number>
```

**Retornos:** Sempre `0`. A implementação real está na camada do repositório.
...
Obtém estatísticas básicas do perfil do cliente.
...
Endpoint do painel de administração otimizado que retorna clientes, estatísticas e paginação em uma única chamada. Reduz viagens de ida e volta ao banco de dados.
...
**Filtros suportados:**
- Pesquisa de texto (ILIKE em 5 campos)
- Status, plano, tipo de conta
- Provedor de autenticação (subconsulta EXISTS)
- Período (criado/atualizado depois/antes)
...
1. **Padrão de exclusão de administrador** – Todas as consultas de listagem usam `LEFT JOIN userRoles / roles WHERE roles.id IS NULL` para excluir consistentemente usuários administradores das visualizações do cliente.

2. **Subconsulta para provedor** - Usa a subconsulta `(SELECT provider FROM accounts WHERE ... LIMIT 1)` em vez de JOIN para evitar a multiplicação de linhas de várias contas OAuth.

3. **Escape SQL adequado** -- Todas as entradas de pesquisa de texto são escapadas para caracteres especiais ILIKE (`%`, `_`, `\`) para evitar injeção de SQL por meio de termos de pesquisa.

4. **Tratamento de semana ISO** -- `getWeeklyEngagementData` usa um auxiliar personalizado `getISOWeekString` que corresponde ao `to_char(date, 'IYYY-IW')` do PostgreSQL para tratamento correto de semana e ano ISO nos limites do ano.
