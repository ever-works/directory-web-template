...
title: Repositório de análise administrativa
sidebar_label: Repositório de Análise
sidebar_position: 18
---

# Repositório otimizado para Admin Analytics

A classe `AdminAnalyticsOptimizedRepository` fornece consultas analíticas de alto desempenho para o painel de administração. Ele usa SQL bruto via Drizzle ORM com um cache TTL na memória para reduzir a carga do banco de dados para métricas acessadas com frequência.

**Arquivo fonte:** `template/lib/repositories/admin-analytics-optimized.repository.ts`
...
Retorna tendências mensais de registro de usuários com totais cumulativos.

```ts
async getUserGrowthTrends(months: number = 12): Promise<UserGrowthTrend[]>
```

|Parâmetro|Tipo|Padrão|Alcance|
|-----------|------|---------|-------|
|`months`|`number`| `12` | 1 -- 120 |

**Padrão SQL:** usa um CTE (`monthly_counts`) para agrupar `users.created_at` por `DATE_TRUNC('month', ...)` e, em seguida, uma função de janela cumulativa (`SUM(...) OVER (ORDER BY month_start)`) para calcular totais em execução. Os resultados são limitados ao número de meses solicitado e ordenados de forma crescente.
...
Retorna itens classificados pela contagem total de votos.
...
**Padrão SQL:** usa um CTE com função de janela `ROW_NUMBER() OVER (ORDER BY COUNT(*) DESC)` para classificar itens por contagem de votos e, em seguida, filtra até o N superior.
...
Executa todas as quatro consultas analíticas em paralelo via `Promise.all` para desempenho ideal de carregamento do painel.
...
Todos os parâmetros de entrada são limitados a limites seguros antes da execução.
...
Retorna a taxa de acertos do cache e as estatísticas de tempo de consulta.
...
> **Observação:** Atualmente retorna dados simulados. Na produção, integre-se ao monitoramento de desempenho real.
