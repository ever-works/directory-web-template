...
title: "Aprofundamento do esquema de comentários"
sidebar_label: "Esquema de comentários"
sidebar_position: 54
---

# Aprofundamento do esquema de comentários

## Visão geral

O sistema de comentários permite que os usuários deixem comentários e avaliações sobre os itens. Os comentários estão vinculados a `client_profiles` (não diretamente a `users`), incluem um campo de classificação e suportam exclusão reversível via `deletedAt`. O subsistema de moderação (`reports` e `moderation_history`) fornece relatórios de conteúdo e rastreamento de ações administrativas.
...
|`contentType`|`content_type`|`text (enum)`|Não| - |`item`, `comment`|
|`contentId`|`content_id`|`text`|Não| - |ID do conteúdo denunciado|
|`reason`|`reason`|`text (enum)`|Não| - |Veja enum abaixo|
|`details`|`details`|`text`|Sim| - |Detalhes fornecidos pelo usuário|
|`status`|`status`|`text (enum)`|Não|`'pending'`|Veja enum abaixo|
|`resolution`|`resolution`|`text (enum)`|Sim| - |Veja enum abaixo|
|`reportedBy`|`reported_by`|`text`|Não| - |FK -> `client_profiles.id` (CASCADA)|
|`reviewedBy`|`reviewed_by`|`text`|Sim| - |FK -> `users.id` (DEFINIR NULO)|
|`reviewNote`|`review_note`|`text`|Sim| - |Nota de revisão do administrador|
|`createdAt`|`created_at`|`timestamp`|Não|`now()`| - |
|`updatedAt`|`updated_at`|`timestamp`|Não|`now()`| - |
|`reviewedAt`|`reviewed_at`|`timestamp`|Sim| - | - |
|`resolvedAt`|`resolved_at`|`timestamp`|Sim| - | - |
...
|Nome|Colunas|Tipo|
|---|---|---|
|`reports_content_type_idx`|`contentType`|Árvore B|
|`reports_content_id_idx`|`contentId`|Árvore B|
|`reports_status_idx`|`status`|Árvore B|
|`reports_reported_by_idx`|`reportedBy`|Árvore B|
|`reports_created_at_idx`|`createdAt`|Árvore B|
|`reports_content_type_content_id_idx`|`(contentType, contentId)`|Árvore B composta|
...
### Obter comentários para um item
...
### Excluir um comentário suavemente
