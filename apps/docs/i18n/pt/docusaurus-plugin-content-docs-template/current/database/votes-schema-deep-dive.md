...
title: "Aprofundamento do esquema de votos"
sidebar_label: "Esquema de Votos"
sidebar_position: 55
---

# Aprofundamento do esquema de votos

## Visão geral

O sistema de votos implementa um mecanismo de voto positivo/negativo para itens. Cada usuário (identificado por seu registro `client_profiles`) pode emitir exatamente um voto por item, reforçado por um índice composto exclusivo. Os votos podem ser alternados entre voto positivo e voto negativo.
...
- **Os votos fazem referência a perfis de clientes, não a usuários.** Isso é consistente com a tabela de comentários. Os usuários devem ter um registro `client_profiles` para votar.
- **Os itens são identificados por slug.** A coluna `item_id` armazena o slug do item do Git CMS, não uma chave estrangeira do banco de dados.
- **Nenhuma tabela separada de contagem de votos.** As contagens de votos são agregadas no momento da consulta usando `count()`. Isso negocia o custo da consulta por consistência (sem contadores obsoletos).
- **O campo `updatedAt` rastreia as alterações de votação.** Quando um usuário muda de voto positivo para voto negativo, `updatedAt` é atualizado enquanto `createdAt` preserva o tempo de votação original.
