...
title: Arquitetura de Pagamento
...
O modelo Ever Works implementa um sistema de pagamento independente de provedor que oferece suporte a quatro provedores de pagamento: **Stripe**, **LemonSqueezy**, **Polar** e **Solidgate**. A arquitetura usa um padrão de fábrica com instâncias de provedor singleton, permitindo a troca de provedor em tempo de execução.
...
Cada provedor de pagamento implementa o mesmo `PaymentProviderInterface` :
...
Essa interface garante que a troca de provedores não exija nenhuma alteração no código de chamada.

## PaymentProviderFactory

A fábrica cria instâncias de provedor com base em uma string de tipo de provedor:
...
O `PaymentService` envolve uma única instância do provedor por trás de uma API limpa. Ele delega todas as chamadas ao provedor subjacente:
...
O serviço também define constantes do plano de pagamento:
...
O gerenciador lida com o ciclo de vida singleton e a troca de provedor de tempo de execução. Persiste o provedor selecionado em `localStorage` :
...
O gerenciador lida com a renderização do lado do servidor padronizando o provedor padrão configurado quando `localStorage` não está disponível:
...
## PaymentProviderManager (camada de configuração)

O `PaymentProviderManager` fornece um padrão de acesso singleton alternativo com inicialização lenta por provedor e validação de configuração:
...
O módulo de configuração exporta funções auxiliares para acesso rápido aos provedores:
...
A classe interna `ConfigManager` carrega e valida variáveis de ambiente para todos os provedores. A configuração de cada provedor é validada no primeiro acesso, não na inicialização:
...
A validação é acionada uma vez por provedor e gera erros descritivos listando as variáveis ​​ausentes.
...
Cada provedor retorna um conjunto de componentes de UI para renderizar formulários de pagamento:
...
Para adicionar um quinto provedor de pagamento, siga estas etapas:

### 1. Crie a classe do provedor
...
O módulo de tipos de pagamento inclui auxiliares de formatação:
