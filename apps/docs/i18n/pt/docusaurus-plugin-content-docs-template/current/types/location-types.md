...
Definições de configuração armazenadas em `works.yml` usando a nomenclatura `snake_case`. Usado ao analisar a seção `settings.location` do arquivo de configuração.
