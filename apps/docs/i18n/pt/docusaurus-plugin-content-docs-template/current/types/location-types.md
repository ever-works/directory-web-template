...
Definições de configuração armazenadas em `works.yaml` usando a nomenclatura `snake_case`. Usado ao analisar a seção `settings.location` do arquivo de configuração.
