...
API категорий предоставляет облегченную общедоступную конечную точку для проверки существования категорий в системе. Категории извлекаются из уровня контента (CMS на основе Git), а не из базы данных, что делает эту конечную точку доступной даже без подключения к базе данных.

**Исходный файл:** `template/app/api/categories/exists/route.ts`

## Сводка конечных точек

|Метод|Путь|Авторизация|Описание|
|--------|------|------|-------------|
|ПОЛУЧИТЬ|`/api/categories/exists`|Нет|Проверьте, существуют ли категории|
