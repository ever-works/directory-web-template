...
API коллекций предоставляет общедоступную конечную точку для проверки наличия в базе данных каких-либо активных коллекций. Коллекции — это курируемые группы элементов, управляемые через панель администратора и хранящиеся в базе данных через репозиторий коллекций.
