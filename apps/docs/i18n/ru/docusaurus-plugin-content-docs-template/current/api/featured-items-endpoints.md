...
title: "Рекомендуемые элементы Конечные точки API"
sidebar_label: "Рекомендуемые товары"
sidebar_position: 18
---

# Рекомендуемые элементы Конечные точки API

API избранных элементов предоставляет общедоступную конечную точку для получения элементов, которые были выделены для заметного отображения на веб-сайте. Рекомендуемые элементы поддерживают порядок, даты истечения срока действия и активные/неактивные состояния.

**Исходный файл:** `template/app/api/featured-items/route.ts`

## Сводка конечных точек

|Метод|Путь|Авторизация|Описание|
|--------|------|------|-------------|
|ПОЛУЧИТЬ|`/api/featured-items`|Нет|Получайте активные избранные элементы для публичного показа|
...
|Файл|Цель|
|------|---------|
|`template/app/api/featured-items/route.ts`|Конечная точка общедоступных избранных элементов|
|`template/lib/db/schema.ts`|`featuredItems` определение таблицы|
