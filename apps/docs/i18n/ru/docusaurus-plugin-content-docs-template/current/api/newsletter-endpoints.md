...
Система поддерживает двух поставщиков электронной почты, настроенных через `works.yml` и переменные среды:
...
Провайдер выбирается на основе поля `mail.provider` в `works.yml`. Конфигурация электронной почты создается динамически из конфигурации приложения с помощью `createEmailConfig()`.
