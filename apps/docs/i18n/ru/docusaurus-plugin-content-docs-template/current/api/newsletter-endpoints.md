...
Система поддерживает двух поставщиков электронной почты, настроенных через `works.yaml` и переменные среды:
...
Провайдер выбирается на основе поля `mail.provider` в `works.yaml`. Конфигурация электронной почты создается динамически из конфигурации приложения с помощью `createEmailConfig()`.
