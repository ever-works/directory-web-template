...
`lib/config-manager.ts` управляет файлом `works.yml` внутри каталога `.content/` (клонированным из хранилища данных). Он считывает и записывает конфигурацию YAML, а также автоматически фиксирует и отправляет изменения в репозиторий Git, используя `isomorphic-git`. Используется для настроек, которые администраторы могут изменять во время выполнения (нумерация страниц, навигация, верхний/нижний колонтитул).
