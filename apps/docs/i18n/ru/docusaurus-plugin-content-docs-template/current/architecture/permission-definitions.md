...
title: Определения разрешений
sidebar_label: Определения разрешений
sidebar_position: 32
---

# Определения разрешений

Шаблон включает в себя детальную систему разрешений, организованную в определения на основе ресурсов, группы с удобным пользовательским интерфейсом и утилиты управления состоянием. Эта система контролирует доступ к функциям администрирования, управлению контентом и аналитике.
...
## Определения разрешений (`definitions.ts`)

Все разрешения соответствуют соглашению об именах `resource:action`. Объект `PERMISSIONS` является единственным источником истины:
...
Тип `Permission` автоматически является производным от объекта `PERMISSIONS`, обеспечивая безопасность типов без дублирования вручную:
...
Два встроенных определения ролей предоставляют разумные значения по умолчанию:
...
## Группы разрешений (`groups.ts`)

Группы разрешений организуют разрешения для отображения в компонентах пользовательского интерфейса администратора, таких как редакторы ролей:
...
## Государственные утилиты разрешений (`utils.ts`)

Эти функции управляют состоянием переключения разрешений в пользовательском интерфейсе администратора (например, формы создания или редактирования ролей):

### Создание состояния разрешения

Преобразуйте массив разрешений в логическую карту для пользовательских интерфейсов на основе флажков:
...
Определите, что было добавлено и удалено при сравнении наборов разрешений:
...
Проверьте, содержат ли два массива разрешений одинаковые разрешения независимо от порядка:
...
Поиск разрешений по ключевому слову:
...
### Проверка разрешений в компоненте
...
- `lib/permissions/definitions.ts` — константы разрешений, типы и роли по умолчанию.
- `lib/permissions/groups.ts` - Группы пользовательского интерфейса и помощники по форматированию
- `lib/permissions/utils.ts` - Утилиты управления состоянием
- `lib/guards/` — средства защиты маршрутов и компонентов, использующие разрешения.
