...
title: Справочник по структуре проекта
sidebar_label: Структура проекта
sidebar_position: 4
---

# Справочник по структуре проекта

В этом документе представлена полная информация о структуре каталогов шаблонов Ever Works. Шаблон представляет собой **монорепозиторий Turborepo**, управляемый с помощью рабочих пространств pnpm. Основное приложение — это приложение Next.js 16, использующее App Router, TypeScript, Drizzle ORM и Tailwind CSS, расположенное в `apps/web/`.

## Обзор верхнего уровня (корень Monorepo)
...
Основное приложение находится в `apps/web/`. Все исходные каталоги, которые ранее находились в корне репозитория, теперь находятся здесь.
...
В шаблоне используется **Turborepo** для оркестрации сборки и **рабочие пространства pnpm** для управления зависимостями в монорепозитории.

|Инструмент|Цель|Конфигурация|
|------|---------|---------------|
|Турборепо|Оркестрация задач, кеширование, параллельные сборки|`turbo.json`|
|рабочие области pnpm|Управление зависимостями, связывание рабочих областей|`pnpm-workspace.yaml`|
|`packages/tsconfig`|Общие базовые конфигурации TypeScript|Расширяется каждым приложением|
|`packages/eslint-config`|Общие пресеты плоской конфигурации ESLint 9|Импортируется каждым приложением|

### Приложения для рабочей области

- **`apps/web`** -- Основное приложение Next.js 16 (листинг каталогов, панель администратора, маршруты API).
- **`apps/web-e2e`** -- Комплексные тесты драматурга, ориентированные на `apps/web`. Выделено собственное рабочее пространство для изолированных зависимостей и независимых запусков CI.
- **`apps/docs`** — сайт документации Docusaurus v3 для самого шаблона.
