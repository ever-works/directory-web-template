...
Настройки геолокации определены в разделе `settings.location` файла `works.yaml` репозитория контента:
...
Исходный формат из `works.yaml`, определённый в `lib/types/location.ts`:
