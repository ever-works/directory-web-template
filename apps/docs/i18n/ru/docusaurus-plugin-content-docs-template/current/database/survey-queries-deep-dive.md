...
title: Подробное изучение опросных запросов
sidebar_label: Подробное изучение опросных запросов
sidebar_position: 68
---

# Подробное изучение опросных запросов

Комплексное руководство по всем функциям запросов к базе данных, связанным с опросами, включая CRUD опроса, управление ответами, нумерацию страниц, фильтрацию и подсчет ответов.

## Обзор

Уровень запросов опросов управляет опросами и ответами на них:

- **`survey.queries.ts`** – полный CRUD опроса с мягким удалением, постраничный список с фильтрами, числом ответов и статусом завершения JOIN, CRUD ответа на опрос с фильтрацией по диапазону дат и подсчетом ответов.

Опросы поддерживают два типа: `global` (для всего сайта) и `item` (связанные с конкретным элементом). Ответы связаны как с опросом, так и, при необходимости, с элементом, что позволяет собирать отзывы по каждому элементу.
...
Получает опрос по его URL-адресу с возможностью исключения определенного идентификатора опроса (полезно для проверки уникальности во время обновлений).
...
|------------|----------|----------|-------------------------------------------|
|`slug`|`string`|Да|Ссылка на опрос|
|`ignoreId`|`string`|Нет|Идентификатор опроса, который нужно исключить из поиска|

**Возврат:** Опрос или `null`, если не найден.
...
**Возврат:** Созданная запись опроса (через `RETURNING`)
...
Программное удаление опроса осуществляется путем установки `deletedAt` на текущую временную метку.
...
**Примечание.** Все запросы на чтение фильтруются по `deleted_at IS NULL`, поэтому обратимое удаление опросов эффективно скрыто от всех функций поиска.
...
Получает ответы на опросы с разбивкой по страницам с дополнительной фильтрацией по элементу, пользователю и диапазону дат.
...
|-------------|----------|----------|---------|-----------------------------------|
|`surveyId`|`string`|Да| --      |Идентификатор опроса, на который нужно получить ответы|
|`itemId`|`string`|Нет| --      |Фильтровать по элементу|
|`userId`|`string`|Нет| --      |Фильтровать по пользователю|
|`startDate`|`string`|Нет| --      |Фильтровать ответы после этой даты|
|`endDate`|`string`|Нет| --      |Фильтровать ответы до этой даты|
|`page`|`number`|Нет| `1`     |Номер страницы|
|`limit`|`number`|Нет| `10`    |Результаты на странице|

**Возвраты:** Список ответов с разбивкой на страницы
...
**Примечания по производительности.** Запросы данных и количества выполняются параллельно через `Promise.all`.
...
Получает общее количество ответов на опрос.
...
**Возвраты:** Число ответов указывается в виде числа.
