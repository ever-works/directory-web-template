---
id: user-types
title: Определения типов пользователей и профилей
sidebar_label: Типы пользователей
sidebar_position: 2
---

# Определения типов пользователей и профилей

**Источник:** `lib/types/user.ts` и `lib/types/profile.ts`

Эти модули определяют типы пользователей-администраторов (аутентификация и управление) и общедоступные профили пользователей (отображение и портфолио).
...
Данные пользователя, предназначенные только для аутентификации, хранятся в таблице `users`. Это минимальное представление пользователя, используемое для потоков аутентификации.
...
Полные данные пользователя, включая информацию профиля от `clientProfiles`. Это полное представление пользователя, используемое на панели администратора.
...
**Отличие от `AuthUserData`:** `UserData`, `username`, `name`, `role`, `status` и `created_by` являются обязательными (необязательными).

#### `CreateUserRequest`

Полезная нагрузка для создания нового пользователя-администратора.
...
Полезная нагрузка для обновления существующего пользователя. Все поля являются необязательными.
...
Разбитый на страницы ответ для списков пользователей аутентификации.
...
Разбитый на страницы ответ для полных списков пользователей.
...
Оболочка ответа одного пользователя.
...
Параметры запроса для фильтрации и разбивки списков пользователей на страницы.
...
Расширенные пользовательские данные с дополнительным полем подсчета статистики.
...
Полная схема проверки для создания пользователя:
...
Частичная схема обновлений (кроме пароля):
...
Введите защиту, которая сужает строку до `UserStatus`:
...
Генерирует UUID для нового пользователя:
...
Форматирует `Date` как строку, совместимую с YAML (`YYYY-MM-DD HH:mm`):
