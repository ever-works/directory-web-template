...
本文档提供了 Ever Works 模板目录结构的完整参考。该模板是使用 pnpm 工作区管理的 **Turborepo monorepo**。主要应用程序是使用 App Router、TypeScript、Drizzle ORM 和 Tailwind CSS 的 Next.js 16 应用程序，位于`apps/web/`。
...
主要应用程序位于`apps/web/`。以前位于存储库根目录的所有源目录现在都驻留在此处。
...
该模板使用 **Turborepo** 进行构建编排，使用 **pnpm 工作区**进行跨 monorepo 的依赖项管理。

|工具|目的|配置|
|------|---------|---------------|
|涡轮雷波|任务编排、缓存、并行构建|`turbo.json`|
|pnpm 工作区|依赖管理、工作区链接|`pnpm-workspace.yaml`|
|`packages/tsconfig`|共享 TypeScript 基本配置|由每个应用程序扩展|
|`packages/eslint-config`|共享 ESLint 9 平面配置预设|由每个应用程序导入|

### 工作区应用程序

- **`apps/web`** -- 主要的 Next.js 16 应用程序（目录列表、管理面板、API 路由）。
- **`apps/web-e2e`** -- 针对`apps/web` 的剧作家端到端测试。分为自己的工作区，以实现隔离的依赖关系和独立的 CI 运行。
- **`apps/docs`** -- 模板本身的 Docusaurus v3 文档站点。
