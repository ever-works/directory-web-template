...
export class AdminBulkActionsPage extends BasePage {
	...
	constructor(page: Page) {
		...
		// "Bulk actions"). Match strictly — don't fall back to any
		// `role="toolbar"` because the page also has other always-on
		// toolbars (header, filters) that would never satisfy
		// `toBeHidden()`.
		this.bulkActionBar = page.locator('[role="toolbar"][aria-label*="ulk" i]').first();
