...
export function TechnologyCard({ 
  ...
  const getColorClasses = (color: string) => {
    const colorMap = {
      ...
      gray: {
        ...
        hover: "hover:border-gray-300 dark:hover:border-white/[0.08]"
      }
    };
  ...
  const popularityData = {
    ...
    low: { 
      ...
      color: "bg-gray-100 dark:bg-white/[0.03] text-gray-800 dark:text-gray-200",
      ring: "opacity-90",
      percentage: 45
    }
  };
  ...
  return (
    <div className={`group relative bg-white/90 dark:bg-white/[0.03] backdrop-blur-xs rounded-2xl border-2 transition-all duration-500 hover:shadow-2xl hover:-translate-y-2 ${colors.border} ${colors.hover} ${popularityData[popularity].ring}`}>
      {/* Popularity Badge */}
      ...
      <div className="p-6">
        ...
        <div className="flex-1 min-w-0">
            ...
            <div>
                ...
                {version && (
                    <span className="text-xs px-2 py-1 bg-slate-100 dark:bg-white/[0.08] rounded-full font-mono border border-slate-200 dark:border-white/[0.08]">
                    v{version}
                  </span>
                )}
                </div>
        ...
        <div className="mb-4">
          ...
          <div className="h-2 bg-slate-200 dark:bg-white/[0.08] rounded-full overflow-hidden">
            <div 
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {getFeatureTags(name).slice(0, 3).map((tag, index) => (
              <span 
                key={index} 
              className="px-2 py-1 text-xs bg-slate-100 dark:bg-white/[0.08] rounded-md border border-slate-200 dark:border-white/[0.08] font-medium text-slate-700 dark:text-slate-300 transition-all duration-300 hover:bg-slate-200 dark:hover:bg-white/[0.08]"
              >
                {tag}
              </span>
            ))}
          {getFeatureTags(name).length > 3 && (
            <span className="px-2 py-1 text-xs bg-slate-100 dark:bg-white/[0.08] rounded-md border border-slate-200 dark:border-white/[0.08] font-medium text-slate-700 dark:text-slate-300">
        ...
        <Button
          ...
          variant="outline"
          size="sm"
          className={`w-full border-slate-300 dark:border-white/[0.08] text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-white/[0.06] transition-all duration-300 ${
            isExpanded ? 'bg-slate-50 dark:bg-white/[0.05]' : ''
          }`}
        >
        ...
        {isExpanded && (
          <div className="mt-6 pt-6 border-t border-slate-200 dark:border-white/[0.06]">
            {/* Tabs */}
            <div className="flex gap-1 mb-4 bg-slate-100 dark:bg-white/[0.08] rounded-lg p-1">
              {[
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {alternatives.map((alternative, index) => (
                      <div key={index} className="flex items-center justify-between p-2 bg-slate-50 dark:bg-white/[0.08] rounded-lg border border-slate-200 dark:border-white/[0.08]">
