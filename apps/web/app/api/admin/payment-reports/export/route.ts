...
import { listAllPaymentRecords, summarizePayments } from '@/lib/db/queries/payment-report.queries';
 ...
 *       400: { description: "Bad request - unsupported format or invalid filter" }
...
export async function GET(request: Request) {
	try {
		...
		const [records, summary] = await Promise.all([
			listAllPaymentRecords(parsed.filters),
