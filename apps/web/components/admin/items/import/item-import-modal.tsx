...
export function ItemImportModal({ isOpen, onClose }: ItemImportModalProps) {
	...
	return (
		...
		>
			{/* Step Indicator */}
			<div className="flex items-center justify-center gap-2 px-6 py-3 border-b border-gray-200 dark:border-white/[0.04]">
				{STEP_KEYS.map((s, i) => (
					<div key={s.key} className="flex items-center gap-2">
						...
						{i < STEP_KEYS.length - 1 && (
							<div className="w-8 h-px bg-gray-300 dark:bg-white/[0.08] mx-1" />
						)}
					</div>
				))}
			</div>
