...
export function TagForm({ tag, mode, onSubmit, onCancel, isLoading = false }: TagFormProps) {
  ...
  const baseInputClasses = "w-full px-3 py-2 border rounded-lg text-sm transition-colors focus:outline-none focus:ring-2 focus:ring-gray-900 dark:focus:ring-white focus:border-transparent";
  const errorInputClasses = "border-red-300 bg-red-50 dark:bg-red-900/20 dark:border-red-700";
  const normalInputClasses = "border-gray-200 dark:border-white/[0.06] bg-white dark:bg-white/[0.05] text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500";
  const disabledInputClasses = "bg-gray-50 dark:bg-white/[0.02] cursor-not-allowed opacity-60";
  ...
  return (
    <div className="bg-white dark:bg-white/[0.03] rounded-xl shadow-xl border border-gray-200 dark:border-white/[0.06]">
      {/* Header */}
      <div className="px-6 py-4 border-b border-gray-200 dark:border-white/[0.06] bg-gray-50/60 dark:bg-white/[0.02]">
      ...
      <form onSubmit={handleSubmit} className="p-6 space-y-5">
        ...
        <div className="space-y-1.5">
          <div className="flex items-center justify-between">
            ...
            <span className={`text-xs tabular-nums ${formData.name.length > TAG_VALIDATION.NAME_MAX_LENGTH * 0.9 ? 'text-orange-500 dark:text-orange-400' : 'text-gray-400 dark:text-gray-500'}`}>
        ...
        <div className="space-y-1.5">
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-gray-900 dark:focus:ring-white focus:ring-offset-2 dark:focus:ring-offset-gray-900 ${
                formData.isActive
        ...
        {/* Actions */}
        <div className="flex items-center justify-end gap-3 pt-4 border-t border-gray-200 dark:border-white/[0.06] bg-gray-50/60 dark:bg-white/[0.02] -mx-6 -mb-6 px-6 pb-6 mt-5">
          <Button
            type="button"
            variant="flat"
            onPress={onCancel}
            ...
            className="px-5 py-2 text-sm font-medium text-gray-700 dark:text-gray-200"
          >
            {t('CANCEL')}
          </Button>
          <Button
            ...
            className="px-5 py-2 text-sm font-medium bg-gray-900 dark:bg-white text-white dark:text-gray-900 hover:bg-gray-800 dark:hover:bg-gray-100 shadow-sm transition-colors"
          >
