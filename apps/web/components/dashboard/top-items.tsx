      ...
      <div className="bg-white dark:bg-white/[0.03] rounded-xl shadow-xs border border-gray-200 dark:border-white/[0.06] p-6">
        <div className="animate-pulse">
          ...
          <div className="space-y-4">
            {[...Array(3)].map((_, i) => (
              <div key={i} className="bg-gray-200 dark:bg-white/[0.08] h-16 rounded-lg"></div>
            ))}
          </div>
        </div>
      </div>
      ...
      <div className="space-y-4">
        {items.length === 0 ? (
          ...
          items.map((item, index) => (
            <div 
              key={item.id}
              className="flex items-center justify-between p-4 bg-gray-50 dark:bg-white/[0.04] rounded-lg border border-gray-200 dark:border-white/[0.06]"
            >
