...
export function Avatar({ src, alt = '', fallback, size = 'sm', className }: AvatarProps) {
	...
	const tailwindUnit = Number.parseInt(sizeMap[size].split(' ')[0].replace('h-', ''), 10);
	...
	return (
		<div
			className={`rounded-full text-xs bg-linear-to-br from-theme-primary-600 to-theme-primary-700 flex items-center justify-center text-white font-medium ${sizeMap[size]} ${
				className ?? ''
			}`}
			title={alt}
		>
