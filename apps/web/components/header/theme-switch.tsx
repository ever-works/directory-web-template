...
const ThemePreview: React.FC<{ themeKey: ThemeKey }> = React.memo(
  ({ themeKey }) => {
    ...
    return (
      <button
        ...
        className={`w-full p-3 rounded-xl transition-all duration-200 flex items-center gap-3 ${
          isActive
            ? "bg-blue-50 dark:bg-white/5 border-2 border-blue-400"
            : "bg-gray-50 dark:bg-white/5 border-2 border-gray-200 dark:border-white/8 hover:border-gray-300 dark:hover:border-white/8"
        }`}
        onClick={handleClick}
      ...
      >
        ...
        {/* Preview */}
        <div className="w-12 h-8 bg-gray-100 dark:bg-white/8 rounded-sm overflow-hidden relative">
  ...
  if (!mounted) {
    return (
      <div className={`animate-pulse ${className}`}>
        <div className="h-8 bg-gray-200 dark:bg-white/8 rounded-sm" />
      </div>
    );
  }
  ...
  return (
    <div className={`relative ${className}`} ref={popoverRef}>
      ...
      {isOpen && (
        <div
          id={panelId}
          className="absolute right-0 mt-2 p-3 w-80 bg-white dark:bg-white/3 border border-gray-200 dark:border-white/6 rounded-lg shadow-lg z-50"
        >
