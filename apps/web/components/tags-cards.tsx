  ...
  const renderTagCard = (tag: Tag) => {
    ...
    return (
      ...
      >
        {/* Loading overlay */}
        {loadingTag === tag.id && (
          <div className="absolute inset-0 border-1 border-theme-primary-200 dark:border-white/[0.06] bg-white/80 dark:bg-[#0a0a0a]/80 backdrop-blur-md rounded-2xl flex items-center justify-center z-50 transition-opacity duration-300">
            <Spinner size="lg" color="primary" />
          </div>
        )}
        <div
          className={cn( "group overflow-hidden hover:overflow-hidden relative rounded-2xl p-2 shadow-none border-1 border-theme-primary-200 dark:border-white/[0.06] bg-white dark:bg-white/[0.03] backdrop-blur-md transition-all duration-700 h-full",
