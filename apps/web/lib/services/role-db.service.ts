...
export class RoleDbService {
	...
	private async getRolesWithPermissions(roleIds?: string[]): Promise<RoleData[]> {
		...
		if (roleIds && roleIds.length > 0) {
			roleQuery = db
				.select()
				.from(roles)
				.where(and(isNull(roles.deletedAt), inArray(roles.id, roleIds), eq(roles.tenantId, tenantId)));
		}
	...
	private async updateRolePermissions(roleId: string, newPermissions: Permission[]): Promise<void> {
		...
		await db.transaction(async (tx) => {
			...
			const permissionRecords = await tx
				...
				.from(permissions)
				.where(and(inArray(permissions.key, newPermissions), eq(permissions.tenantId, tenantId)));
			...
			return await db.transaction(async (tx) => {
				...
				if (data.permissions && data.permissions.length > 0) {
					const permissionRecords = await tx
						.select({ id: permissions.id, key: permissions.key })
						.from(permissions)
						.where(and(inArray(permissions.key, data.permissions), eq(permissions.tenantId, tenantId)));
	...
	async findRoles(options: RoleListOptions = {}): Promise<{
		...
		try {
			...
			const filters: SQL[] = [isNull(roles.deletedAt), eq(roles.tenantId, tenantId)]; // Always exclude deleted roles and filter by tenant
			if (status) {
