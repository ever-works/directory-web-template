...
export function applyFeaturedFlags(items: ItemData[], featuredItems: FeaturedItem[]): ItemData[] {
	const featuredSlugs = new Set(featuredItems.filter((item) => item.isActive).map((item) => item.itemSlug));

	if (featuredSlugs.size === 0) return items;

	return items.map((item) => (featuredSlugs.has(item.slug) && !item.featured ? { ...item, featured: true } : item));
}

/**
 * Sort items to prioritize featured items at the top
 */
export function sortItemsWithFeatured(items: ItemData[], featuredItems: FeaturedItem[]): ItemData[] {
 ...
 */
export function isItemFeatured(itemSlug: string, featuredItems: FeaturedItem[]): boolean {
	return featuredItems.some((item) => item.itemSlug === itemSlug && item.isActive);
}

/**
 * Get featured item data for a specific item
 */
export function getFeaturedItemData(itemSlug: string, featuredItems: FeaturedItem[]): FeaturedItem | undefined {
	return featuredItems.find((item) => item.itemSlug === itemSlug && item.isActive);
}
 ...
 */
export function filterActiveFeaturedItems(featuredItems: FeaturedItem[]): FeaturedItem[] {
	const now = new Date();
