...
function MenuItems({ user, profilePath, onItemClick, onNavigationStart, isNavigating }: MenuItemsComponentProps) {
	...
	const translations = useMemo(
		() => ({
			...
			organizationSettings: t('organization.ORGANIZATION_SETTINGS'),
			organizationDesc: t('organization.MANAGE_ORGANIZATION_DESC')
		}),
		[t]
	);
	...
	const MenuItem = ({
		...
		return (
			...
			>
				<div
					className={`flex items-center justify-center w-10 h-10 mr-3 rounded-xl bg-linear-to-br ${gradientFrom} ${gradientTo} group-hover:from-opacity-80 group-hover:to-opacity-80 transition-all duration-200`}
				>
	...
	if (isAdmin) {
		return (
			<div
				className="max-h-96 overflow-y-auto scrollbar-thin scrollbar-thumb-rounded scrollbar-thumb-neutral-300 dark:scrollbar-thumb-neutral-700 [&::-webkit-scrollbar]:w-1"
			...
			>
				...
				<MenuItem
					...
					gradientTo="to-gray-100 dark:from-slate-900/30 dark:to-gray-900/30"
	...
	return (
		<div
			className="max-h-64 overflow-y-auto scrollbar-thin scrollbar-thumb-rounded scrollbar-thumb-neutral-300 dark:scrollbar-thumb-neutral-700 [&::-webkit-scrollbar]:w-1"
			style={{ scrollbarWidth: 'thin' }}
		>
			<MenuItem
				href={profilePath}
			...
			<MenuItem
				...
				icon={Building2}
				title={translations.organizationSettings}
				description={translations.organizationDesc}
				gradientFrom="from-blue-100"
				gradientTo="to-indigo-100 dark:from-blue-900/30 dark:to-indigo-900/30"
				iconColor="text-theme-primary-600 dark:text-theme-primary-400"
			/>
