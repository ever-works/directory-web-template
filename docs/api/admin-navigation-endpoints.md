...
These admin endpoints manage custom site navigation links and the geographic location index. Navigation endpoints allow configuring custom header and footer links stored in `.works/works.yml`. Location index endpoints manage the spatial index used for geographic analytics and map features.
...
Retrieves the `custom_header` and `custom_footer` navigation items from the site's `.works/works.yml` file. Returns empty arrays if no custom navigation is configured.
...
Updates the custom header or footer navigation items in `.works/works.yml`. Validates each item's path format to prevent XSS attacks via dangerous URL schemes.
...
- **Config Storage:** Navigation items are stored in `.works/works.yml` under `custom_header` and `custom_footer` keys, persisted via `configManager.updateNestedKey()`.
