...
The admin settings API provides endpoints for reading and modifying site configuration stored in `works.yaml`. This includes general settings and map provider status. All endpoints require admin authentication.
...
Retrieves the complete `settings` section from the site's `works.yaml` file.
...
The exact shape of the `settings` object depends on the site's `works.yaml` configuration. The endpoint returns whatever is stored under the `settings` key.
...
Updates a single setting value within the `settings` section of `works.yaml`. The key is automatically scoped to the `settings` namespace (e.g., providing key `"theme"` updates `settings.theme` in the configuration file).
...
The update is persisted via `configManager.updateNestedKey()`, which modifies the underlying `works.yaml` file. The key is automatically prefixed with `settings.` before being passed to the config manager.
...
- **Storage:** Settings are stored in a YAML configuration file (`works.yaml`)
