...
The Featured Items API provides a public endpoint for retrieving items that have been highlighted for prominent display on the website. Featured items support ordering, expiration dates, and active/inactive states.

**Source file:** `template/app/api/featured-items/route.ts`

## Endpoint Summary

| Method | Path | Auth | Description |
|--------|------|------|-------------|
| GET | `/api/featured-items` | None | Get active featured items for public display |
...
| File | Purpose |
|------|---------|
| `template/app/api/featured-items/route.ts` | Public featured items endpoint |
| `template/lib/db/schema.ts` | `featuredItems` table definition |
