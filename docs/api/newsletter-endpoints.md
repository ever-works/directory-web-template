...
The system supports two email providers configured via `works.yaml` and environment variables:
...
The provider is selected based on the `mail.provider` field in `works.yaml`. Email configuration is built dynamically from the app config using `createEmailConfig()`.
