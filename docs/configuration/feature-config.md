...
The `ConfigManager` class at `lib/config-manager.ts` manages the `works.yaml` file from the Git-based CMS repository. It reads legacy `works.yml`, `config.yaml`, and `config.yml` files as fallbacks for older repositories, writes changes to canonical `works.yaml`, and commits configuration changes.
