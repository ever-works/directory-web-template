...
1. **Service**: `lib/services/bookmark.service.ts` -- orchestrates query calls and adds business logic (see [How to Add a Service](/guides/how-to-add-a-service))
2. **API Route**: `app/api/bookmarks/route.ts` -- thin handler that validates input, calls the service, and returns HTTP responses (see [How to Add an API Endpoint](/guides/how-to-add-an-api-endpoint))
...
- [How to Add a Service](/guides/how-to-add-a-service) -- building the business logic layer on top of your new table
- [How to Write Database Migrations](/guides/how-to-write-database-migrations) -- advanced migration techniques
- [Repository Patterns](/architecture/repository-patterns) -- when to use repositories vs. queries
- [Database Health Check](/guides/database-health-check) -- monitoring your database in production
