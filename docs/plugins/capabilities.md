...
- [Plugin Providers Reference](./providers.md) — the parallel **per-export** reference paired with `providers.ts`; documents the *TypeScript shape* of every provider interface this page covers (one section per `AuthProvider` / `PaymentProvider` / `AnalyticsProvider` / `SearchProvider` / `ContentSource` / `MapsProvider` / `NewsletterProvider` / `NotificationsProvider` / `AIProvider`), every member with its type and nullability, the `(string & {})` literal-with-fallback trick on `PaymentProvider.id`, the `'ui-slot' = never` lockout in `CapabilityProviderMap`, the read / write surface, and the failure matrix that maps every observable failure (compile-time, `setup`-throw, fan-out swallow, single-lookup propagation, runtime malformed shape) onto the layer that surfaces it. Where this page is the *runtime contract*, that page is the *type-system contract*.
- [Reference Plugin (`@ever-works/plugin-demo`)](./plugin-demo.md) — per-source-file reference paired with `packages/plugin-demo/src/`; the demo plugin declares **only** the `'ui-slot'` capability and is the in-tree worked example of the lockout this page enforces (since `'ui-slot'` is typed as `never` in `CapabilityProviderMap`, the demo plugin has no `providers` field at all). Documents the demo-specific failure manifestations that anchor the capability contract for plugins that mix capabilities + slots.
