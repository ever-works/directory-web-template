...
- [Plugin Manifest Reference](manifest.md) — per-field reference paired with `manifest.ts`; documents the `templateRange` field whose semver check the boot-time decision tree above runs first, the `config` schema run during phase 4, the `defaultEnabled` flag the runtime falls back to when no `plugin_settings` row exists, and the `adminToggleable` flag the admin UI honours when surfacing enable / disable for this lifecycle.
- [Plugin Definition Reference](plugin.md) — per-export reference paired with `plugin.ts`; documents the `DirectoryPlugin<C>` shape the boot phases populate, the `PluginContext` injected into `setup` and every slot component, the `SlotComponentProps` slot-component contract, and the `setup` / `teardown` hook semantics that anchor the enable / disable phases.
- [Plugin Providers Reference](providers.md) — per-export reference paired with `providers.ts`; documents every concrete provider interface attached during phase 6 (registration), the `CapabilityProviderMap` mapped type the registry's `get<C>` / `list<C>` lookups consume, and the failure matrix that anchors the lifecycle's `setup`-throw → `LoadPluginsResult.rejected[name].reason: 'setup'` rule.
- [Reference Plugin (`@ever-works/plugin-demo`)](plugin-demo.md) — per-source-file reference paired with `packages/plugin-demo/src/`; the demo plugin is the in-tree worked example of the lifecycle phases on this page (its `defaultEnabled: true` flag drives the boot path that reaches register without an admin override, and its `'header.right'` slot contribution disappears the instant the admin REST endpoint disables it because the slot host respects the registry's enable state on every render — exactly the disable behaviour described in phases 5 and 7 above).
- [Spec 002 — Plugin Architecture](https://github.com/ever-works/directory-web-template/tree/develop/docs/spec/002-plugin-architecture)
