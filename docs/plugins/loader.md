...
- [Plugin Providers Reference](./providers.md) — per-export reference paired with `providers.ts`; documents the nine concrete provider interfaces a plugin's `setup` hook may throw inside (and how that throw routes to `LoadPluginsResult.rejected[name].reason: 'setup'`), and the `CapabilityProviderMap` mapped type that the loader passes through to the registry untouched.
- [Reference Plugin (`@ever-works/plugin-demo`)](./plugin-demo.md) — per-source-file reference paired with `packages/plugin-demo/src/`; the demo plugin's `ConfigSchema` (`z.object({ enabled: z.boolean().default(true), greeting: z.string().default('Demo plugin loaded') })`) is the in-tree worked example of a `manifest.config` schema this loader runs `.parse(merged)` against, including the no-config-sources path the e2e smoke check exercises (both fields fall back to their defaults). Documents the demo-specific failure manifestations of every loader rejection path (Zod-rejected `enabled: 'yes'` / `greeting: 42`, `templateRange '>=0.1 <1.0'` mismatch, duplicate-name throw if a second plugin tries to register `name: 'demo'`).
