...
- [Reference Plugin (`@ever-works/plugin-demo`)](./plugin-demo.md) — per-source-file reference paired with the bundled reference plugin under `packages/plugin-demo/src/`; documents the at-a-glance manifest summary, the file map (`config.ts` → manifest schema, `Header.tsx` → slot component, `index.tsx` → `defineDirectoryPlugin` composition), the per-line walk-through of `ConfigSchema` / `DemoConfig` / `DemoHeaderBadge` / the `defineDirectoryPlugin` invocation, the three call sites the demo plugin participates in (loader, registry, slot host), the failure matrix that maps demo-plugin manifestations onto the loader / registry / slot-host failure surfaces, the replace-the-demo-plugin recipe, and the evolution checklist.
- [Plugin SDK Public Surface Reference](./sdk-public-surface.md) — per-source-file reference paired with `packages/plugin-sdk/src/index.ts`; documents every public name the SDK barrel re-exports split by kind (value vs. type-only), the `package.json#exports` sub-path map (`.`, `./capabilities`, `./slots`) and why the rest of the SDK source files are intentionally not addressable as sub-paths, the per-line walkthrough that ties each line of the barrel to its documentation impact, the value-vs-type contract that locks moving a name across the boundary as a breaking change, the failure matrix that maps barrel-level mistakes (non-public sub-path import, value-vs-type mis-import, lost `C` inference, capability not added to `CAPABILITIES`, dropped `sideEffects` flag) onto the layer that surfaces them, and the public-surface change checklist that ties any addition / removal back to Spec Kit and the `docs/log.md` entry.
- [Plugin Runtime Public Surface Reference](./runtime-public-surface.md) — per-source-file reference paired with `packages/plugin-runtime/src/index.ts`; documents every public name the runtime barrel re-exports (`PluginRegistry`, `loadPlugins`, `mergeConfigSources`, `PluginConfigSources`, `LoadPluginsResult`, `SlotHost`, `SlotHostProps`, `createTestRegistry`) split by kind (value vs. type-only) and grouped by the three concerns the runtime owns (the host-app boot pipeline, the React render surface, and the unit-test helper), the `package.json#exports` sub-path map (`.`, `./registry`, `./SlotHost`, `./loader`, `./testing`) and the rationale for keeping the four narrowed sub-paths so server actions can import the registry without dragging React into the server bundle and tests can import `createTestRegistry` without spinning up a JSDOM environment, the per-line walkthrough that ties each line of the barrel to its documentation impact (the JSDoc preamble's React-aware-only-in-`SlotHost` / owns-registry-loader-host / cross-link invariants, the registry value re-export, the loader value and type re-exports, the slot-host value and props re-exports, and the testing-helper value re-export), the value-vs-type contract that locks moving a name across the boundary as a breaking change, the failure matrix that maps barrel-level mistakes (non-public sub-path import, value-vs-type mis-import, treeshake-stripped `PluginRegistry`, registry-instance mismatch between loader and `<SlotHost />`, dropped `sideEffects` flag, React leaking into a server bundle) onto the layer that surfaces them, and the public-surface change checklist that ties any addition / removal back to Spec Kit and the `docs/log.md` entry.
