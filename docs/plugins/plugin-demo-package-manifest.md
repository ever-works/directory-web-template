...
- [Plugin Package TypeScript Configurations](./plugin-tsconfigs.md)
  ...
  demo packages. The demo's `tsconfig.json` is byte-identical to the
  SDK's and runtime's; the JSX flag (`"react-jsx"`) is the load-bearing
  setting that lets `Header.tsx` author JSX without an explicit React
  import.
