...
- [Plugin Providers Reference](./providers.md) — per-export reference paired with `providers.ts`; documents the parallel **capability** surface `<SlotHost />` does *not* render (because `'ui-slot'` is the only capability that has no programmatic provider — typed as `never` in `CapabilityProviderMap`), and the read / write surface that explicitly lists `<SlotHost />` as not reading from or writing to any provider.
- [Reference Plugin (`@ever-works/plugin-demo`)](./plugin-demo.md) — per-source-file reference paired with `packages/plugin-demo/src/`; the demo plugin contributes `DemoHeaderBadge` to `'header.right'` and is the in-tree worked example of every contract on this page: a real `<SlotHost slotId="header.right" registry={...} />` call site that renders the demo plugin's badge, the `data-plugin="demo"` / `data-testid="demo-plugin-badge"` selectors `<SlotHost />`-driven tests target, the disabled-config short-circuit (`if (!ctx.config.enabled) return null;`) `<SlotHost />` exercises through merged config sources rather than registry-level unregistration, and the Fragment-only output rule `<SlotHost />` enforces around the demo's `<span>` markup.
