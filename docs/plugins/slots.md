...
- [Reference Plugin (`@ever-works/plugin-demo`)](./plugin-demo.md) — per-source-file reference paired with `packages/plugin-demo/src/`; the demo plugin contributes to the `'header.right'` slot id documented on this page and is the in-tree worked example of the `slots: { 'header.right': DemoHeaderBadge }` binding shape, the `SlotComponentProps<DemoConfig>` typed prop, and the `data-plugin="demo"` / `data-testid="demo-plugin-badge"` selectors that slot-rendering tests can target.
