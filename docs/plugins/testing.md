...
- [Reference Plugin (`@ever-works/plugin-demo`)](./plugin-demo.md) — per-source-file reference paired with `packages/plugin-demo/src/`; the demo plugin can be passed directly to `createTestRegistry({ plugins: [demoPlugin] })` as a real, in-tree fixture for every worked example on this page (its `defaultEnabled: true` flag makes the happy-path render assertion straightforward, its `if (!ctx.config.enabled) return null;` short-circuit gives the disable round-trip a deterministic empty state to assert against, and its declared-but-not-implemented `'ui-slot'` capability is the canonical example of the `'ui-slot' = never` lockout that the testing helper inherits from the SDK).
