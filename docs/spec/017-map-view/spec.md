...
- **Directory operators** who toggle the feature on through `works.yaml`.
...
- G-2: Add a **Map** entry to the primary header navigation. The entry
  is gated by a single boolean in `works.yaml` and is hidden when
...
flag in works.yaml so that I can ship it without writing code.
