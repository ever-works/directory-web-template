...
| 026 | [`client-settings-preferences-section`](026-client-settings-preferences-section/spec.md) | proposed      | Mount the `SettingsModal` controls (Layout, Container Width, Pagination Style + demo-only DB mode / Checkout Provider / DB warning) inline as a new **Preferences** section on `/client/settings`; modal stays as-is for shortcut access |
