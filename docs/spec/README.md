...
| 026 | [`ew627-client-dashboard-wiring`](026-ew627-client-dashboard-wiring/spec.md) | in-progress  | EW-627: `/client/dashboard` UI ↔ API wiring fixes — drop hardcoded `Shares` slice, make `viewsAvailable` a real signal, add error UI on map/geo cards, placeholder when location features are disabled, i18n the engagement-chart and top-items headings, tighten `StatsCard.value` to `number`, trim `engagementOverview` to the user's real activity window |
| 027 | [`fix-post-register-autologin`](027-fix-post-register-autologin/spec.md) | shipped       | Fix prod-only "register → not actually logged in → stuck on signin" bug: move sign-in into the `signUp` server action so the session cookie ships in the same response, plus a `useRef` guard on the legacy client-side auto-login `useEffect` to prevent the double-fire that caused `TypeError: Failed to fetch` |
...
| 033 | [`client-profile-security-billing`](033-client-profile-security-billing/spec.md) | proposed | EW-648: add two owner-only read-only blocks to `/client/profile/[username]` — **Security & Status** (email verification, 2FA, account status, member-since → security settings) and **Billing & Plans** (plan, account type, currency → billing settings / pricing). Pure server components over data already on the profile row, sourced from the unprojected `rawProfile`, gated by `effectiveIsOwn`; 30 new `profile` i18n keys fully translated across all 21 locales |
| 034 | [`client-billing-ui-consistency`](034-client-billing-ui-consistency/spec.md) | in-progress | UI-only alignment of `/client/settings/profile/billing` (and its `components/settings/billing/**`) with the client dashboard design system: neutral palette (no `slate-*`/`theme-primary-*`), `bg-white dark:bg-white/3` cards, monochrome icon tiles, dashboard `StatsCard` KPIs + underline tabs; plus wiring previously-dead actions (CSV export, status filters, invoice download, LemonSqueezy cancel, etc.) — no data changes (EW-649) |
| 035 | [`admin-header-profile-link`](035-admin-header-profile-link/spec.md) | proposed | EWW-5: the admin branch of the shared `ProfileButton` dropdown (`profile-button/menu-items.tsx`) omitted the "Your Profile" link the non-admin branch has, so an admin had no link to their own profile; add it (reuses existing `common.YOUR_PROFILE` keys). The `/admin` panel header keeps its `Admin Panel` label unchanged |
| 036 | [`docker-build-publish-workflow`](036-docker-build-publish-workflow/spec.md) ([plan](036-docker-build-publish-workflow/plan.md), [tasks](036-docker-build-publish-workflow/tasks.md)) | shipped | Add dev/stage/prod GHCR-first Docker build/publish workflows for the template's root `Dockerfile`, with optional Docker Hub and DigitalOcean Container Registry pushes when credentials are configured |
| 037 | [`item-detail-perf`](037-item-detail-perf/spec.md) | proposed | Fix blank/slow first paint on `/[locale]/items/[slug]`: **stream** the bottom "Similar Products" carousel behind its own Suspense boundary (React 19 `use()` + server promise) so the full-catalogue similarity scan no longer blocks first paint; plus persist the scored result in Next's Data Cache (`getCachedSimilarItems`) and run the page's item + translations loads concurrently. Final markup/ordering unchanged |
| 038 | [`k8s-deploy-probes`](038-k8s-deploy-probes/spec.md) | in-progress | Harden the k8s deploy manifest template's pod health probes: add a `startupProbe` and explicit `timeoutSeconds: 5` + `failureThreshold` on readiness/liveness so a server-rendered `/` no longer trips Kubernetes' 1s default timeout and crash-loops (root cause of 460+ restarts on the first k8s-deployed Work) |
| 039 | [`e2e-git-cms-ci-safe`](039-e2e-git-cms-ci-safe/spec.md) | in-progress | Make authenticated write-flow e2e specs pass in CI: skip git-CMS remote `pull`/`push` (no HTTP timeout, unreachable CI remote) via `isContentGitRemoteDisabled()` (`CI`/`CONTENT_GIT_OFFLINE`), add an in-flight git-service init lock, and make the favorite-toggle page object dismiss the login modal opened by a pre-hydration click. No prod behaviour change; no tests skipped |
| 040 | [`k8s-deploy-runtime-env`](040-k8s-deploy-runtime-env/spec.md) | in-progress | `deploy_k8s.yaml` materializes a `${slug}-runtime-env` Secret (AUTH_SECRET/COOKIE_SECRET/DATABASE_URL/NEXT_PUBLIC_APP_URL/COOKIE_*) the Deployment mounts via `envFrom` — fixes the runtime 500 (`AUTH_SECRET must be set`) on k8s-deployed sites. Platform half = ever-works DeployService.ensureRuntimeEnv |
