...
| 037 | [`item-detail-perf`](037-item-detail-perf/spec.md) | proposed | Speed up `/[locale]/items/[slug]` on cold paths: persist the similar-items computation in Next's Data Cache (shared across instances, survives cold starts) via a new `getCachedSimilarItems` wrapper instead of the per-process in-memory map, and run the page's independent item + translations loads concurrently. No output/markup change |
