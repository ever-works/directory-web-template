...
| 034 | [`admin-header-profile-link`](034-admin-header-profile-link/spec.md) | proposed | EWW-5: admin header (`admin/layout-client.tsx`) was a static `Admin Panel` bar with no profile control; render the shared `ProfileButton` right-aligned, and add the missing "Your Profile" link to the admin branch of `profile-button/menu-items.tsx` (reuses existing `common.YOUR_PROFILE` keys) so an admin can reach their own profile |
