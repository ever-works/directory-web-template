...
| 034 | [`admin-header-profile-link`](034-admin-header-profile-link/spec.md) | proposed | EWW-5: the admin branch of the shared `ProfileButton` dropdown (`profile-button/menu-items.tsx`) omitted the "Your Profile" link the non-admin branch has, so an admin had no link to their own profile; add it (reuses existing `common.YOUR_PROFILE` keys). The `/admin` panel header keeps its `Admin Panel` label unchanged |
