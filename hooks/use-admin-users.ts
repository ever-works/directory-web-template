...
export function useAdminUsers(options: UseAdminUsersOptions = {}): UseAdminUsersReturn {
	const { page: initialPage = 1, limit = 10, role: initialRole = '', status: initialStatus = '' } = options;
	...
	const handleUpdateUser = useCallback(
		async (id: string, data: UpdateUserRequest): Promise<boolean> => {
			try {
	...
	const setStatusFilterString = useCallback(
		(status: string) => {
			setStatusFilter(status as 'active' | 'inactive' | '');
		},
		[setStatusFilter]
