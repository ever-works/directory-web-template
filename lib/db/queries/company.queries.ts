 ...
 */
export async function getCompanyById(id: string, tenantId: string): Promise<Company | null> {
 ...
 * @returns Company or null if not found
 */
export async function getCompanyBySlug(slug: string, tenantId: string): Promise<Company | null> {
 ...
 * @returns Company or null if not found
 */
export async function getCompanyByDomain(domain: string, tenantId: string): Promise<Company | null> {
 ...
 * @returns Company or null if not found
 */
export async function getCompanyByName(name: string, tenantId: string): Promise<Company | null> {
...
export async function listCompanies(params: {
	...
	const { page = 1, limit = 10, search, status, sortBy = 'createdAt', sortOrder = 'desc', tenantId } = params;
	const offset = (page - 1) * limit;

	const whereConditions: SQL[] = [eq(companies.tenantId, tenantId)];
 ...
 */
export async function getCompaniesStats(tenantId: string): Promise<{
	total: number;
	active: number;
	inactive: number;
}> {
 ...
 */
export async function assignCompanyToItem(itemSlug: string, companyId: string, tenantId: string): Promise<ItemCompany> {
	const result = await linkItemToCompany(itemSlug, companyId, tenantId);
 ...
 */
export async function unlinkItemFromCompany(
	itemSlug: string,
	tenantId: string
 ...
 * @returns True if deleted, false otherwise
 */
export async function removeCompanyFromItem(itemSlug: string, tenantId: string): Promise<boolean> {
	const result = await unlinkItemFromCompany(itemSlug, tenantId);
 ...
 */
export async function getCompanyByItemSlug(itemSlug: string, tenantId: string): Promise<Company | null> {
	const normalizedSlug = itemSlug.toLowerCase().trim();
	...
	const [result] = await db
		...
		.where(
			and(
				eq(itemsCompanies.itemSlug, normalizedSlug),
				eq(companies.tenantId, tenantId) // Ensure company belongs to tenant, item-company likely matches too but good to be safe
			)
		)
		.limit(1);
 ...
 * @returns Company or null if not associated
 */
export async function getCompanyForItem(itemSlug: string, tenantId: string): Promise<Company | null> {
	return getCompanyByItemSlug(itemSlug, tenantId);
}
...
export async function getItemsForCompany(
	...
	return listItemsByCompany(companyId, tenantId, params);
}
 ...
 */
export async function itemHasCompany(itemSlug: string, tenantId: string): Promise<boolean> {
	const normalizedSlug = itemSlug.toLowerCase().trim();
