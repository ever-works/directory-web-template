...
export class AdminStatsRepository {
		...
		try {
			...
			const [totalUsersResult, newUsersTodayResult, newUsersWeekResult, newUsersMonthResult] = await Promise.all([
				...
				db
					.select({ count: count() })
					.from(users)
					.where(and(isNull(users.deletedAt), eq(users.tenantId, tenantId), gte(users.createdAt, todayUtc))),
				db
					.select({ count: count() })
					.from(users)
					.where(
						and(isNull(users.deletedAt), eq(users.tenantId, tenantId), gte(users.createdAt, weekStartUtc))
					),
				db
					.select({ count: count() })
					.from(users)
					.where(
						and(isNull(users.deletedAt), eq(users.tenantId, tenantId), gte(users.createdAt, monthStartUtc))
					)
			]);
	...
	async getActivityStats(tenantId: string): Promise<ActivityStats> {
		try {
			const [totalVotesResult, totalCommentsResult, totalViews] = await Promise.all([
				db.select({ count: count() }).from(votes).where(eq(votes.tenantId, tenantId)),
				db
					.select({ count: count() })
					.from(comments)
					.where(and(isNull(comments.deletedAt), eq(comments.tenantId, tenantId))),
	...
	async getNewsletterStats(tenantId: string): Promise<NewsletterStats> {
		try {
			const now = new Date();
			const todayUtc = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate()));
			const weekStartUtc = new Date(todayUtc);
			...
			const [totalSubscribersResult, recentSubscribersResult] = await Promise.all([
				db
					...
					.where(
						and(eq(newsletterSubscriptions.isActive, true), eq(newsletterSubscriptions.tenantId, tenantId))
					),
				db
					.select({ count: count() })
					.from(newsletterSubscriptions)
