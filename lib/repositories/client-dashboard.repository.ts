...
export class ClientDashboardRepository {
	...
	async getStats(userId: string, tenantId: string): Promise<DashboardStats> {
		...
		const clientProfile = await getClientProfileByUserId(userId, tenantId);
		...
		const engagementDistribution = this.calculateEngagementDistribution(
			userItems,
			topItemsEngagement,
			viewsPerItemMap
		);
	...
	private calculateCategoryPerformance(
		...
		categoryStats.forEach((stats, category) => {
			result.push({
				...
				avgEngagement: stats.itemCount > 0 ? Math.round((stats.totalEngagement / stats.itemCount) * 10) / 10 : 0
			});
		});

		// Sort by average engagement (descending) and return top 5
		return result.sort((a, b) => b.avgEngagement - a.avgEngagement).slice(0, 5);
	}
	...
	private calculateEngagementDistribution(
		...
		return itemsWithEngagement.map((item) => ({
			...item,
			percentage: totalEngagement > 0 ? Math.round((item.engagement / totalEngagement) * 100) : 0
		}));
	}
