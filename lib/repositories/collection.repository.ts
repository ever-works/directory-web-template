...
export class CollectionRepository {
	...
	private async getGitService(): Promise<CollectionGitService> {
		if (!this.gitService) {
			...
			if (!gitConfig.token) {
				throw new Error(
					'GitHub token not configured. Please set GH_TOKEN (or GITHUB_TOKEN) for collection sync.'
				);
			}
	...
	async delete(id: string, tenantId: string): Promise<void> {
		...
		for (const item of items) {
			if (Array.isArray(item.collections) && item.collections.includes(id)) {
				...
				await this.itemRepository.update(item.id, { ...item, collections: nextCollections }, tenantId);
			}
		}
	...
	async assignItems(
		...
		try {
			...
			if (itemUpdates.length > 0) {
				// Pass tenantId. But wait, batchUpdate takes (updates, tenantId).
				// Let's check ItemRepository batchUpdate.
				// Assuming it's updated or we loop.
				// If batchUpdate doesn't take tenantId, we might have an issue.
				// But for now replacing to pass it.
				await this.itemRepository.batchUpdate(itemUpdates, tenantId);
			}
	...
	private validateCreateData(data: CreateCollectionRequest): void {
		...
		if (data.id.trim().length > COLLECTION_VALIDATION.ID_MAX_LENGTH) {
			throw new Error(
				`Collection ID must be no more than ${COLLECTION_VALIDATION.ID_MAX_LENGTH} characters long`
			);
		}
		...
		if (!data.name || data.name.trim().length < COLLECTION_VALIDATION.NAME_MIN_LENGTH) {
			throw new Error(
				`Collection name must be at least ${COLLECTION_VALIDATION.NAME_MIN_LENGTH} characters long`
			);
		}

		if (data.name.trim().length > COLLECTION_VALIDATION.NAME_MAX_LENGTH) {
			throw new Error(
				`Collection name must be no more than ${COLLECTION_VALIDATION.NAME_MAX_LENGTH} characters long`
			);
		}

		if (data.description && data.description.trim().length > COLLECTION_VALIDATION.DESCRIPTION_MAX_LENGTH) {
			throw new Error(
				`Description must be no more than ${COLLECTION_VALIDATION.DESCRIPTION_MAX_LENGTH} characters long`
			);
		}
	}
	...
	private validateUpdateData(data: UpdateCollectionRequest): void {
		...
		if (
			...
			data.description.trim().length > COLLECTION_VALIDATION.DESCRIPTION_MAX_LENGTH
		) {
			throw new Error(
				`Description must be no more than ${COLLECTION_VALIDATION.DESCRIPTION_MAX_LENGTH} characters long`
			);
