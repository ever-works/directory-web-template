...
export class ItemRepository {
	...
	async create(data: CreateItemRequest, tenantId: string, auditUser?: AuditUser): Promise<ItemData> {
		...
		try {
			await itemAuditService.logCreation(item, tenantId, auditUser);
		} catch (err) {
	...
	async update(id: string, data: UpdateItemRequest, tenantId: string, auditUser?: AuditUser): Promise<ItemData> {
		...
		if (previousItem) {
			try {
				await itemAuditService.logUpdate(previousItem, updatedItem, tenantId, auditUser);
			} catch (err) {
	...
	async batchUpdate(
		...
		for (const { previous, updated } of auditEntries) {
			try {
				await itemAuditService.logUpdate(previous, updated, tenantId, auditUser);
			} catch (err) {
	...
	async review(id: string, reviewData: ReviewRequest, tenantId: string, auditUser?: AuditUser): Promise<ItemData> {
		...
		if (previousStatus) {
			try {
				await itemAuditService.logReview(item, previousStatus, reviewData.review_notes, tenantId, auditUser);
			} catch (err) {
	...
	async delete(id: string, tenantId: string, auditUser?: AuditUser): Promise<void> {
		const gitService = await this.getGitService();
		...
		if (item) {
			try {
				await itemAuditService.logDeletion(item, tenantId, auditUser, false);
			} catch (err) {
	...
	async softDelete(id: string, tenantId: string, auditUser?: AuditUser): Promise<ItemData> {
		const gitService = await this.getGitService();
		const item = await gitService.softDeleteItem(id);

		// Log soft deletion to audit trail (best-effort)
		try {
			await itemAuditService.logDeletion(item, tenantId, auditUser, true);
		} catch (err) {
	...
	async restore(id: string, tenantId: string, auditUser?: AuditUser): Promise<ItemData> {
		const gitService = await this.getGitService();
		const item = await gitService.restoreItem(id);

		// Log restoration to audit trail (best-effort)
		try {
			await itemAuditService.logRestoration(item, tenantId, auditUser);
		} catch (err) {
