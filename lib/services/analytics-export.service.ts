...
export class AnalyticsExportService {
	...
	async exportUserGrowthTrends(months: number = 12, options: ExportOptions): Promise<ExportResult> {
		...
		const data = await this.repository.getUserGrowthTrends(months, options.tenantId);
	...
	async exportActivityTrends(days: number = 7, options: ExportOptions): Promise<ExportResult> {
		...
		const data = await this.repository.getActivityTrends(days, options.tenantId);
	...
	async exportTopItems(limit: number = 10, options: ExportOptions): Promise<ExportResult> {
		...
		const data = await this.repository.getTopItems(limit, options.tenantId);
	...
	async exportRecentActivity(limit: number = 10, options: ExportOptions): Promise<ExportResult> {
		...
		const data = await this.repository.getRecentActivity(limit, options.tenantId);
	...
	async exportComprehensiveReport(options: ExportOptions): Promise<ExportResult> {
		...
		const [userGrowth, activityTrends, topItems, recentActivity] = await Promise.all([
			this.repository.getUserGrowthTrends(12, options.tenantId),
			this.repository.getActivityTrends(30, options.tenantId),
			this.repository.getTopItems(50, options.tenantId),
			this.repository.getRecentActivity(100, options.tenantId)
		]);
