...
export class UserDbService {
	async readUsers(tenantId: string): Promise<AuthUserData[]> {
		try {
	...
	async createUser(
		data: Pick<CreateUserRequest, 'email' | 'password'> & { tenantId: string }
	...
	async updateUser(id: string, data: UpdateUserRequest, tenantId: string): Promise<AuthUserData> {
		try {
			const now = new Date();
			...
			const result = await db.transaction(async (tx: PgTransaction<any, any, any>) => {
				...
				if (Object.keys(profileUpdateData).length > 1) {
					...
					await tx.update(clientProfiles).set(profileUpdateData).where(eq(clientProfiles.userId, id));
				}
		...
		try {
			const result = await db
				.select({
					...
					active: sql<number>`count(*) filter (where ${clientProfiles.status} = 'active' AND ${clientProfiles.tenantId} = ${tenantId})`,
					inactive: sql<number>`count(*) filter (where ${clientProfiles.status} != 'active' AND ${clientProfiles.tenantId} = ${tenantId})`
				})
				.from(clientProfiles)
	 ...
	 */
	async clientProfileUsernameExists(username: string, tenantId: string, excludeId?: string): Promise<boolean> {
		try {
